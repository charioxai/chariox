...
- Landed: Phase 8 removed the generic `WorkflowRuntimeStore::execute_request` method. Workflow lanes now enter a store operation boundary and the remaining compatibility mutation match is private to the workflow command executor until Phase 8 ownership migration splits workflow state out of the app lock.
