...
Current status: Phase 6 has started. Workflow commands are admitted through bounded per-session `WorkflowRuntime` lanes and publish session/agent-runtime projections from the lane worker after compatibility mutation. Capability jobs already execute behind `CapabilityExecutor`; health now reports executor job pressure. The remaining Phase 6 work is to move workflow run progression and capability result application out of direct `DaemonApp` mutation.
