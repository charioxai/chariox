...
- broaden actor-owned projections beyond focused-agent routing and warmed session/list/history/provider-run/process/prompt-state snapshots so remaining provider/read models can be served without compatibility-store reads on the hot path
- make the remaining provider-health reads projection-first on the hot path
