...
- Landed: `AgentRuntimeProjectionStore` baseline. Session projection refreshes now materialize per-agent active/queued prompt read models, and daemon health exposes those agent-runtime prompt counts separately from the compatibility session projection counters.
