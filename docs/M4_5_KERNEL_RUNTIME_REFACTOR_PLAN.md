...
- Landed: Phase 8 removed the remaining app-level session/workflow request facade shims. The compatibility local API now calls `KernelSessionService::execute_request` and the explicit workflow runtime request executor directly, so `DaemonApp::handle_session_request` and `DaemonApp::handle_workflow_request` no longer exist.
