...
- Landed: Phase 8 provider terminal input now sits behind an explicit `ProviderTerminalInput` boundary. Prompt dispatch/cancel paths call the boundary directly, and the public app method remains only as a compatibility shim for external tests and legacy terminal-input callers.
