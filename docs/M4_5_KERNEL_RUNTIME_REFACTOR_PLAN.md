...
- Landed: Phase 8 provider terminal input now sits behind an explicit `ProviderTerminalInput` boundary. The public app method remains as a compatibility shim, but prompt dispatch/cancel and terminal-input callers now have a named owner for provider-run liveness validation, attachment validation, terminal input recording, and PTY writes.
