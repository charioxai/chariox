...
- finish moving prompt dispatch side effects out of the compatibility app lock now that `PromptStateOwner` is shared with `AgentRuntime`, and keep per-agent prompt projections read-only
...
2. Finish prompt ownership: `PromptStateOwner` now owns active/queued prompt lifecycle mutation, is shared with `AgentRuntime`, and mirrors into `PromptRuntimeState`; next remove the remaining hot app-lock access around provider dispatch, history append, claim acquisition, and compatibility mirroring side effects.
...
| `kernel/agent_actor.rs` | per-agent mailboxes serialize prompt submit/cancel/complete and share `PromptStateOwner` for active-owner and queue-front decisions, but provider dispatch/history/claim/mirror side effects still enter `KernelAgentService` through compatibility app state | next ownership slice: split prompt side effects so the app lock is not needed for normal owner-backed routing and admission |
