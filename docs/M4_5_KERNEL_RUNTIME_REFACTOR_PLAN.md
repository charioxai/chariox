...
- Landed: Phase 8 introduced `KernelWorkflowService` for workflow command mutations. `WorkflowRuntimeStore` now delegates workflow definition, endpoint, node, edge, schema, flush-context, launch-policy, run, queue, watchdog, validation, and ack mutations to the workflow service instead of owning mutation bodies in the kernel actor. The service is still compatibility-backed by `DaemonApp`, but workflow command ownership is now centralized behind the workflow service boundary.
