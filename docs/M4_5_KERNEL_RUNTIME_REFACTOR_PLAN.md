...
- Landed: provider-run startup queue draining now consults the warmed agent-runtime projection for the expected queued prompt before falling back to compatibility queue state, so queued prompt dispatch after late provider startup follows the same actor-owned candidate path as prompt completion.
