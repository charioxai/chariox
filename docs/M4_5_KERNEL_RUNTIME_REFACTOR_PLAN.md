...
- Landed: runtime MCP HTTP now uses the kernel `CommandRouter` as its transport boundary. The MCP server binds from the router-owned config projection and dispatches authenticated workflow runtime tools through the router instead of locking `DaemonApp` directly from the HTTP transport.
