...
- Landed: Phase 8 moved prompt dispatch and abort enqueue success paths onto owned runtime stores. Prompt echo fanout, provider-run/session validation, structured prompt submit/abort enqueue, prompt activity start records, terminal input record storage, remote prompt success echo fanout, and prompt dispatch/abort failure notice recording, and provider-launch failure/switch notice recording now use owned stores; the remaining app-lock work is provider liveness reconciliation, PTY writes, provider process spawn/removal, structured-job reap, and blocked-workflow retry execution until those process/scheduler seams are isolated.
