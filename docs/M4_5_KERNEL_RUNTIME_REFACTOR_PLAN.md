...
- Landed: prompt submit now computes `Start` vs `Queue` admission inside the per-agent mailbox from the warmed agent-runtime projection before entering compatibility mutation. Compatibility outcomes are reconciled against that projection-backed preview so warmed submit admission stays ordered with same-agent commands.
...
- Landed: `AgentRuntimeProjectionStore` retains the queue front and queued count needed for warmed submit/completion routing. Full queued-prompt ownership remains in compatibility session state until the prompt-ownership slice moves it into `AgentRuntime`.
...
2. Finish prompt ownership: move active/queued prompt state and queue advancement fully into `AgentRuntime`, then treat session prompt fields as compatibility projections.
