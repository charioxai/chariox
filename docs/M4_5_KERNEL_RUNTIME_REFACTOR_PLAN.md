...
|------|-------------|----------------|
| `kernel/router.rs` | compatibility fallback reads, projection refresh snapshots, and provider launch/process/catalog cold paths; the generic interactive request fallback is now closed, and relay config/remote-machine registry mutations now have explicit router handlers | keep cold/background reads bounded; remove fallback reads from warmed interactive paths before exit |
