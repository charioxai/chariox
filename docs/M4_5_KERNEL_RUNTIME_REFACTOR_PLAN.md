...
|------|-------------|----------------|
| `kernel/router.rs` | no generic local compatibility fallback remains, and production router code no longer calls `DaemonApp::handle_local_request`; named cold session/list/resolve/state/agent-list, OpenCode provider-run sync, provider launch/process, auth/logout, and capability branches still touch `DaemonApp` until their stores move behind runtime-owned services | keep these cold/compatibility branches explicit and bounded; migrate them one service at a time instead of reintroducing catch-all fallback |
