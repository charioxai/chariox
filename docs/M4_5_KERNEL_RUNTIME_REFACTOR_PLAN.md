...
- Landed: Phase 8 provider liveness now exposes provider-only reconciliation on `ProviderProcessService`; session active-run cleanup is retained in compatibility wrappers and the provider-runtime liveness state seam. This keeps provider run state transitions provider-owned while session pointers remain session-side compatibility effects.
