...
- Landed: agent prompt command execution now goes through an explicit `AgentRuntimeCommandExecutor` boundary. The executor still uses compatibility-backed stores, but prompt mutation details are no longer embedded directly in the mailbox worker loop, which creates the seam for moving submit/cancel/complete onto runtime-owned stores.
