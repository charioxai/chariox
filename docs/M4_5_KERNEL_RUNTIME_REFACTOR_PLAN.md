...
- Landed: Phase 8 managed provider process tracking now goes through an explicit `ProviderProcessTracker` owner for registration and run cleanup. Session end, provider launch, initialization rollback, teardown, and liveness cleanup no longer depend on a broad process-cleanup helper on `DaemonApp`.
