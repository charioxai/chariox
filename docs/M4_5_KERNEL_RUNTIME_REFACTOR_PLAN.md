...
- Landed: Phase 8 introduced `KernelWorkflowService` for workflow graph/config command mutations. `WorkflowRuntimeStore` now delegates workflow definition, endpoint, node, edge, schema, flush-context, and launch-policy mutations to the workflow service instead of owning those mutation bodies in the kernel actor; run/progression/runtime-tool commands remain the next workflow service extraction target.
