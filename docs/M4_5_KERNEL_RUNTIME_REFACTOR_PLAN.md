...
- Landed: Phase 8 `TerminalOutputExecutor` now isolates its remaining app-lock provider-output dependency behind a `ProviderOutputRuntimeBridge`. The executor still uses the compatibility app lock for active provider output, but the app-lock touch point is now a single bridge that owns compatibility pump invocation and projection refresh.
