...
- Landed: `AgentRuntime` now carries an explicit per-agent prompt-state shadow for active/queued prompt ownership migration. The shadow is hydrated from mailbox-owned submit/cancel/complete results, consulted before projection/session/app fallbacks for active-prompt owner routing, and cleaned up with agent/session lanes while compatibility session state remains the mutation mirror.
- Landed: router-observed session snapshots now also refresh the `AgentRuntime` prompt-state shadow, so prompt lifecycle changes that still arrive through compatibility/background paths cannot leave stale active-prompt ownership ahead of projected/session fallbacks.
- Landed: per-agent prompt mailbox results now refresh only the target agent's runtime prompt shadow and agent-runtime projection. Whole-session refresh remains reserved for router-observed session/list snapshots, preventing an older same-session prompt snapshot from stomping another agent lane's newer active/queued state.
- Landed: `AgentRuntimePromptStateStore` now has explicit submit/cancel/complete lifecycle applicators. The agent mailbox applies prompt lifecycle facts directly to runtime prompt state before reconciling from the compatibility session snapshot, reducing reliance on whole-session prompt snapshots as the source of prompt ownership truth.
- Landed: prompt submit now carries an `AgentRuntime` admission preview (`Start` vs `Queue`) computed from per-agent runtime prompt state before entering compatibility mutation. Compatibility outcomes are reconciled against that preview so runtime-owned prompt state can become the source of submit admission without changing external prompt ids or dispatch behavior yet.
...
- Landed: `AgentRuntimePromptState` now retains the full queued prompt list, not only queue front/count. Submit/cancel/complete lifecycle applicators maintain the list and derived front/count summary, preparing queue advancement to move out of compatibility session state.
...
- Landed: runtime prompt-state lifecycle application now activates queued prompts through an explicit queue-activation helper that removes the started prompt by id and refreshes derived queue-front/count fields, rather than relying on a blind pop-front inside completion/cancellation reconciliation.
...
- Landed: agent mailbox prompt lifecycle handling now publishes `AgentRuntimeProjection` directly from the mailbox-owned prompt-state shadow after submit/cancel/complete. Session snapshots still update the compatibility session projection, but the agent prompt read model is no longer immediately overwritten from the compatibility session mirror on those mailbox paths.
...
- Landed: Phase 8 prompt-submit routing can resolve a missing target agent from the single-agent `AgentRuntimeProjection`/runtime prompt state before falling back to the compatibility app lock. This removes another hot-path lock for warmed single-agent sessions, including sessions whose session projection is not yet warm.
...
Current status: Phase 8 is in progress. Warmed prompt submit/cancel/complete routing, target validation, and missing-session rejection, session read/resolve/inspection/history paths, delete/detach lane resolution, direct session-lane absence rejection, attachment-scoped lane validation, session attach/focus/cycle/alias/end absence validation, provider-process teardown projection refresh, async provider-launch session projection refresh, daemon health terminal pressure, relay status/discovery reads, provider catalog/command catalog reads, settled provider-launch guards, non-blocking cold provider-launch cleanup, agent lifecycle refresh, session config/alias updates, projected session-config attachment validation, runtime notice polling through the terminal store and warmed session projection, terminal output absence/no-active-run/inactive-run projection draining, terminal resize absence handling, active-run provider-lane pumping, provider liveness projection refresh from the provider pump, the provider output pump seam, and router-side focus refresh now prefer actor/runtime/session/config projections and avoid compatibility app-lock fallback in the covered success and absence cases. The remaining Phase 8 work is the larger ownership flip: session/agent/workflow command workers still use `DaemonApp` as the mutation mirror, and cold session/provider-auth/configuration paths still use compatibility state until their runtime-owned stores are separated.
