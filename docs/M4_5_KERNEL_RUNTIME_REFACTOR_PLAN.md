...
- Landed: Phase 8 agent request execution is now folded into `KernelAgentService::execute_request`. `DaemonApp::handle_agent_request` has been removed, local compatibility dispatch enters `KernelAgentService` directly, `AgentRuntime` prompt completion calls the service directly, and session-scoped agent spawn/destroy routed through `SessionRuntime` delegates to the same agent service instead of carrying a duplicate mutation match.
