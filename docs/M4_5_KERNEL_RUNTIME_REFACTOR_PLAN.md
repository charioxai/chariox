...
- Landed: capability executor health counters for submitted/running/completed/failed/join-error jobs. `GetDaemonHealth` now exposes capability job pressure separately from actor mailbox pressure.
- Landed: workflow prompt completion/cancellation entrypoints are now isolated behind the app workflow-runtime boundary, so prompt lifecycle callers no longer invoke scheduler runtime progression functions directly.
...
Current status: Phase 6 has started. Workflow commands are admitted through bounded per-session `WorkflowRuntime` lanes and publish session/agent-runtime projections from the lane worker after compatibility mutation. Capability jobs already execute behind `CapabilityExecutor`; health now reports executor job pressure. Workflow prompt completion/cancellation callers now enter through app workflow-runtime methods instead of calling scheduler progression directly. The remaining Phase 6 work is to move the scheduler progression implementation itself and capability result application out of direct `DaemonApp` mutation.
