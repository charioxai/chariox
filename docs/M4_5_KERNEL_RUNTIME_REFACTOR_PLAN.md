...
- Landed: Phase 8 local API prompt/provider coverage now uses the router-backed local API harness for provider auto-launch, direct complete/cancel owner resolution, invalid adapter rejection, active prompt cancellation, and cross-session provider-prompt workspace claim rejection.
...
- Landed: Phase 8 promoted provider prompt workspace claims into a cloneable store and moved prompt dispatch/remote-dispatch failure prompt-owner cleanup, prompt-activity clearing, claim release, and session refresh onto owned runtime stores. The remaining app-lock work in those failure paths is notice/history emission and blocked-workflow retry execution after a claim release.
