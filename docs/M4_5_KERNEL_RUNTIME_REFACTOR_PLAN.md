...
- finish moving the remaining prompt side effects out of the compatibility app lock now that local provider writes/enqueues are off the agent-mailbox acknowledgement path; history append, remote relay prompt dispatch, claim storage, and compatibility mirroring still need ownership boundaries
...
2. Finish prompt ownership: `PromptStateOwner` now owns active/queued prompt lifecycle mutation, is shared with `AgentRuntime`, mirrors into `PromptRuntimeState`, and local provider writes/enqueues run after synchronous claim admission in spawned provider-operation dispatch; next remove the remaining hot app-lock access around history append, remote prompt dispatch, claim storage, and compatibility mirroring side effects.
