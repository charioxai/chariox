...
- Landed: Phase 8 moved prompt dispatch and abort enqueue success paths onto owned runtime stores. Prompt echo fanout, provider-run/session validation, structured prompt submit/abort enqueue, prompt activity start records, terminal input record storage, and remote prompt success echo fanout now use owned stores; the remaining app-lock work is provider liveness reconciliation, PTY writes, and structured-job reap until those process/scheduler seams are isolated.
