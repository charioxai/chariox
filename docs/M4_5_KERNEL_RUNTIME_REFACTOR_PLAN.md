...
- Landed: remaining prompt lifecycle helpers for provider-prompt claim admission, dispatch-failure cleanup, local/remote queue advancement, active-cancel routing, and cancellation finalization now live with the prompt command phases in `app/kernel_agent/prompt_commands.rs`. `kernel_agent.rs` is reduced to the agent request orchestration boundary plus the legacy direct-submit compatibility path.
