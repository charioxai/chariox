...
the PR #363 workspace-materialization change. The goal is parity of the selected
user workspace behavior; managed security and lifecycle controls remain in force.
...
| Arbitrary cwd selection | Uses the selected workspace/worktree path directly. | Retains the selected cwd below the managed runtime command mask and applies the managed isolation boundary. | The mask is a security boundary, not permission to change the selected user workspace. Protected-state behavior remains outside this slice. |
