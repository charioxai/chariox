...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh/background-render/modal-layer policy, app-sidebar DOM projection, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal records/lifecycle, terminal transport lifecycle/status/target projection, kernel-directory projection policy, waiting-room session lifecycle/menu policy, slash-command parsing, provider-run profile projection, freeform dialog/session-label/prompt-sizing/kernel-row/template/default-agent/pane-shell/live-desk/turn-clock/agent-reference/desk/config/command-center state, freeform footer config/update policy, freeform agent projection, agent substitute snapshot parsing, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,589 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands, daemon health projection, local caller identity projection, relay diagnostic projection, runtime MCP proxy dispatch, cloud relay connection refresh/request dispatch, relay peer lease/projection execution, workspace worktree command policy, agent controls, agent utilities, focus projection refresh, remote relay inventory, capability dispatch, provider process control, waiting-room control, session read projection/inspection/collaboration, pairing dispatch, user config/credential dispatch, slice dispatch, session membership authorization, terminal output execution/projection, provider launch pending tracking, provider visibility policy, response redaction, and many router executors are responsibility modules. `runtime/router.rs` is still the main command router and is 7,953 lines.
- Latest verified slices: OSS slice dispatch moved to `slice_command_executor.rs`; Cloud app-sidebar DOM projection moved to `terminal/app-sidebar-renderer.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- app-sidebar-renderer` (648 tests), `pnpm -r --if-present lint`, and `git diff --check`.
