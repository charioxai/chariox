...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh policy, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal records/lifecycle, terminal transport lifecycle/status/target projection, kernel-directory projection policy, waiting-room session lifecycle/menu policy, slash-command parsing, freeform dialog/session-label/prompt-sizing/kernel-row/template/turn-clock/agent-reference/desk state, freeform footer config policy, freeform agent projection, agent substitute snapshot parsing, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,755 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands, daemon health projection, agent controls, agent utilities, focus projection refresh, remote relay inventory, capability dispatch, provider process control, waiting-room control, session read projection/inspection, session membership authorization, terminal output execution/projection, provider launch pending tracking, provider visibility policy, response redaction, and many router executors are responsibility modules. `runtime/router.rs` is still the main command router and is 8,350 lines.
- Latest verified slices: OSS daemon health projection moved to `daemon_health_projection.rs`; Cloud slash-command parsing moved to `terminal/slash-command-state.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test` (631 tests), `pnpm -r --if-present lint`, and `git diff --check`.
