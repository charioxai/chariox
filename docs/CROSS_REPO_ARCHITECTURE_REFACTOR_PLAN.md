...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh/background-render/modal-layer/workspace-suggestion/session-action-menu/workspace-search/worktree-refresh/create-alias/session-alias-edit/agent-alias-edit/agent-workspace-edit/agent-option-picker state/projection, app-sidebar DOM projection, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal lifecycle/transport/status/target projection, kernel-directory projection policy, provider-run profile projection, freeform state/projection/session-alias/dialog-init/kernel-request policy, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,313 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands/dispatch, daemon health projection/dispatch, local caller identity projection, relay diagnostics/config dispatch, runtime MCP proxy dispatch, cloud relay dispatch, relay peer lease/projection execution, workspace dispatch/worktree policy, history dispatch, provider auth/catalog/process/run dispatch, agent control/utility dispatch, focus projection refresh, remote inventory/registry dispatch, capability dispatch, waiting-room control, session read/projection/inspection/collaboration, pairing/user-config/credential/slice dispatch, membership authorization, terminal output execution/projection, provider launch tracking, provider visibility policy, and response redaction are responsibility modules. `runtime/router.rs` is still the main command router and is 7,678 lines.
- Latest verified slices: OSS daemon-health dispatch moved into `runtime/daemon_health_projection.rs`; Cloud freeform dialog kernel request mapping moved to `terminal/freeform-agent-dialog-requests.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- freeform-agent-dialog-requests` (717 tests).
