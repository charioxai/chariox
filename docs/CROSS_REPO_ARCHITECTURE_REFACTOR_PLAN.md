...
- Latest verified batch: shared session lifecycle, attachment, membership, invite, workspace link, alias, and session-state request builders moved into `packages/kernel-client/src/ipc-session-requests.ts`, with `ipc-requests.ts` retained as a compatibility barrel.
