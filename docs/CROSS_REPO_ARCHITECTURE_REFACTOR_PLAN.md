...
- Latest verified batch: workflow failure recording and mailbox routing moved into `scheduler/runtime/failures.rs`; `scheduler/runtime.rs` is 1,525 lines.
- Latest gates for owned files: `cargo test --manifest-path apps/kernel/Cargo.toml scheduler --lib -- --test-threads=1`, rustfmt check on owned files, and scoped diff check pass. Cloud API typecheck and focused API service tests pass; full Cloud API test is currently blocked by dirty Cloud web client changes expecting `terminal-runtime.js`. Full kernel `cargo fmt --check` is blocked by unrelated existing formatting drift outside these slices.
