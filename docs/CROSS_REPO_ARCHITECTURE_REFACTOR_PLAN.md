...
- Latest verified batch: stdio MCP supervision/framing moved into `provider/mcp_proxy/stdio.rs`; `provider/mcp_proxy.rs` is down to 476 lines.
- Latest gates for owned files: `cargo test --manifest-path apps/kernel/Cargo.toml provider::mcp_proxy --lib -- --test-threads=1`, `cargo test --manifest-path apps/kernel/Cargo.toml provider --lib -- --test-threads=1`, scoped rustfmt, and scoped diff check pass. Full Cloud API test is still blocked by dirty Cloud web client changes expecting `terminal-runtime.js`; full kernel `cargo fmt --check` is blocked by unrelated formatting drift outside these slices.
