...
- OSS modules now cover router composition/actors, command metadata/admission/dispatch, relay/cloud/session bridges, Cloud API HTTP transport/session collaboration, projection store/invariant/session-ref/workspace-coordination policies, runtime state views, prompt/workflow/provider/session/agent/profile/capability/attachment ownership, capability executor health/admission, prompt activity/transcript fan-out, local prompt admission/dispatch/cancellation, prompt queue advancement, prompt skill context, provider focus sync, provider reload policy/pending polling/relaunch tasks, provider launch request building/failure recovery/liveness/substitute handling, provider-switch context handoff reconstruction, runtime notice fan-out, managed I/O argument normalization/workspace identity/parser/patch planning/patch application/whole-file application/file state/edit result/reservation policy/remote state conversion/remote patch/remote whole-file application, runtime interactions, remote prompt submission/lifecycle/worker relay submission, workflow access guards/queries/turn admin/publication CRUD/pairing credentials, workflow endpoints/settings/node topology/edge topology/turn prompts/completion evidence/prompt failure handling/blocked-claim retry/run request execution/queued launch advancement/run resume/launch scheduling admin/runtime console tools/runtime output tools, tool dispatch, response refresh/redaction, agent lane queueing/command execution, session actor agent/terminal adapters, session membership scope policy, and responsibility-owned tests. Next targeted OSS hotspots: `runtime/capability_executor.rs` 437 lines, `runtime/state/tool_dispatch/managed_io_remote_dispatch.rs` 504 lines, `runtime/history_requests.rs` 497 lines, and `runtime/workspace_repo_files.rs` 461 lines. Reduced coordinators: `runtime/capability_executor/health.rs` 115 lines, `runtime/projection/session_state_projection.rs` 181 lines, `runtime/projection/session_state_projection/invariant.rs` 208 lines, `runtime/projection/session_state_projection/resolution.rs` 96 lines, `runtime/projection/session_state_projection/workspace_coordination.rs` 81 lines, `runtime/cloud_api_client.rs` 169 lines, `runtime/agent_actor.rs` 295 lines, `runtime/router.rs` 207 lines, `runtime/session_actor.rs` 136 lines, `runtime/session_actor/store.rs` 255 lines, `runtime/session_membership.rs` 187 lines, `runtime/state/provider.rs` 264 lines, `runtime/state/provider_reload.rs` 219 lines, `runtime/state/provider_reload_pending_runtime.rs` 65 lines, `runtime/state/provider_relaunch_runtime.rs` 73 lines, `runtime/state/provider_launch_runtime.rs` 166 lines, `runtime/state/provider_launch_failure_runtime.rs` 213 lines, `runtime/state/provider_launch_owned_state.rs` 211 lines, `runtime/state/context_handoff.rs` 203 lines, `runtime/state/context_handoff/builder.rs` 302 lines, `runtime/state/agent_config_owned_state.rs` 203 lines, `runtime/state/agent_profile_owned_state.rs` 171 lines, `runtime/state/capability_owned_state.rs` 108 lines, `runtime/state/prompt_dispatch.rs` 237 lines, `runtime/state/prompt_activity_owned_state.rs` 235 lines, `runtime/state/provider_prompt_settlement_runtime.rs` 296 lines, `runtime/state/remote_prompt_dispatch_runtime.rs` 160 lines, `runtime/state/workflow_admin.rs` 58 lines, `runtime/state/workflow_access_owned_state.rs` 138 lines, `runtime/state/workflow_query_owned_state.rs` 53 lines, `runtime/state/workflow_turn_admin_owned_state.rs` 69 lines, `runtime/state/workflow_completion_owned_state.rs` 258 lines, `runtime/state/workflow_completion_snapshot_owned_state.rs` 122 lines, `runtime/state/workflow_prompt_failure_owned_state.rs` 95 lines, `runtime/state/workflow_publication_owned_state.rs` 93 lines, `runtime/state/workflow_publication_pairing_owned_state.rs` 276 lines, `runtime/state/workflow_launch_owned_state.rs` 167 lines, `runtime/state/workflow_launch_queue_owned_state.rs` 125 lines, `runtime/state/workflow_resume_owned_state.rs` 68 lines, `runtime/state/workflow_tool.rs` 259 lines, `runtime/state/workflow_console_tool.rs` 112 lines, `runtime/state/workflow_output_tool.rs` 192 lines, `runtime/state/workflow_request_runtime_state.rs` 291 lines, `runtime/state/workflow_run_request_runtime_state.rs` 191 lines, `runtime/state/workflow_scheduling_owned_state.rs` 122 lines, `runtime/state/workflow_dispatch.rs` 228 lines, `runtime/state/workflow_definition_owned_state.rs` 125 lines, `runtime/state/workflow_node_owned_state.rs` 279 lines, `runtime/state/prompt.rs` 192 lines, `runtime/state/prompt_cancellation_owned_state.rs` 253 lines, `runtime/state/managed_io.rs` 161 lines, `runtime/state/managed_io/patch.rs` 185 lines, `runtime/state/managed_io/remote.rs` 251 lines, `runtime/state/provider_output_runtime.rs` 230 lines, `runtime/state/mod.rs` 281 lines.
- Latest verified batch: OSS capability executor health counters and blocking-task admission moved into `runtime/capability_executor/health.rs`.
- Latest gates for owned files: `cargo test --manifest-path apps/kernel/Cargo.toml capability_executor --lib -- --test-threads=1`, `cargo test --manifest-path apps/kernel/Cargo.toml capability --lib -- --test-threads=1`, rustfmt check on owned files, and diff check pass. Cloud API typecheck and focused API service tests pass; full Cloud API test is currently blocked by dirty Cloud web client changes expecting `terminal-runtime.js`. Full kernel `cargo fmt --check` is blocked by unrelated existing formatting drift outside these slices.
