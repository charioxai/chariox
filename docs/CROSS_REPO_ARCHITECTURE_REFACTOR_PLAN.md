...
- Latest verified batch: provider service behavioral tests moved into `provider/service/tests.rs`; `service.rs` is now a 55-line production composition root.
- Latest gates for owned files: `cargo test --manifest-path apps/kernel/Cargo.toml provider::service --lib -- --test-threads=1`, scoped rustfmt check, and scoped diff check pass. Full Cloud API test is currently blocked by dirty Cloud web client changes expecting `terminal-runtime.js`; full kernel `cargo fmt --check` is blocked by unrelated formatting drift outside these slices.
