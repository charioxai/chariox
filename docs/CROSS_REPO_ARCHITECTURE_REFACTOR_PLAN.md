...
- Cloud API boundary split is in place: `server.ts` is route composition; relay-kernel bootstrap, browser cloud sessions, and runtime-token issuance delegate to focused service use cases; route/helper/contract files are domain-owned.
...
- Latest verified batch: Cloud browser cloud-session creation moved to `apps/api/src/browser-cloud-session-service.ts`.
