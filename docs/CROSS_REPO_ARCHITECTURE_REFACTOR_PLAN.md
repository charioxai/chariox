...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh policy, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal records/lifecycle, terminal transport lifecycle/status/target projection, kernel-directory projection policy, waiting-room session lifecycle/menu policy, freeform dialog/session-label/prompt-sizing/kernel-row state, freeform agent projection, agent substitute snapshot parsing, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,807 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands, agent utilities, provider process control, waiting-room control, session read projection/inspection, session membership authorization, terminal output execution/projection, provider launch pending tracking, provider visibility policy, response redaction, and many router executors are responsibility modules. `runtime/router.rs` is still the main command router and is 8,452 lines.
- Latest verified slices: OSS agent utility archive-config lookup moved to `agent_utility_executor.rs`; Cloud freeform kernel-row/workflow-create policy moved to `terminal/freeform-kernel-row-state.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test` (615 tests), `pnpm -r --if-present lint`, and `git diff --check`.
