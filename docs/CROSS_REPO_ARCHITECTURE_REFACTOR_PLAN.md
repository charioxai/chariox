...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh/background-render/modal-layer/workspace-suggestion/session-action-menu/workspace-search/worktree-refresh/create-alias/session-alias-edit/agent-alias-edit/agent-workspace-edit/agent-option-picker state, app-sidebar DOM projection, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal lifecycle/transport/status/target projection, kernel-directory projection policy, provider-run profile projection, freeform state/projection/session-alias policy, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,355 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands, daemon health projection, local caller identity projection, relay diagnostics, runtime MCP proxy dispatch, cloud relay dispatch, relay peer lease/projection execution, workspace dispatch/worktree policy, history dispatch, provider auth/catalog/process/run dispatch, agent control/utility dispatch, provider catalog dispatch, focus projection refresh, remote inventory/registry dispatch, capability dispatch, waiting-room control, session read/projection/inspection/collaboration, pairing/user-config/credential/slice dispatch, membership authorization, terminal output execution/projection, provider launch tracking, provider visibility policy, and response redaction are responsibility modules. `runtime/router.rs` is still the main command router and is 7,661 lines.
- Latest verified slices: OSS provider catalog fast-path dispatch moved into `runtime/provider_catalog_control.rs`; Cloud sidebar agent option-picker state moved to `terminal/sidebar-agent-option-picker-state.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- sidebar-agent-option-picker-state` (705 tests).
