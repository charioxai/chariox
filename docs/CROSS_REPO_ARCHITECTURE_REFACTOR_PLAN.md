...
- Latest verified batch: provider launch request/result, resume state, execution/permission/write-access config, and runtime MCP binding moved into `provider/launch_contract.rs`; `provider/types.rs` is down to 608 lines.
- Latest gates for owned files: `cargo test --manifest-path apps/kernel/Cargo.toml provider --lib -- --test-threads=1`, scoped rustfmt check, and scoped diff check pass. Full Cloud API test is still blocked by dirty Cloud web client changes expecting `terminal-runtime.js`; full kernel `cargo fmt --check` is blocked by unrelated formatting drift outside these slices.
