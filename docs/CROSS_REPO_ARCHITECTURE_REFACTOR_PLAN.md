...
- Latest verified batch: session-scoped operational history reads and legacy fallback checks moved into `history/operational_session.rs`; `history.rs` is 958 lines.
- Latest gates for owned files: `cargo test --manifest-path apps/kernel/Cargo.toml history --lib -- --test-threads=1`, rustfmt check on owned files, and scoped diff check pass. Cloud API typecheck and focused API service tests pass; full Cloud API test is currently blocked by dirty Cloud web client changes expecting `terminal-runtime.js`. Full kernel `cargo fmt --check` is blocked by unrelated existing formatting drift outside these slices.
