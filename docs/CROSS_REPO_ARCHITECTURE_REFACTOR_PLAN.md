...
- Latest verified batch: context command rendering moved into `packages/kernel-client/src/shell-context-command.ts`; shared mode/permission parsing moved into `shell-agent-policy.ts`; `shell-executor.ts` is down to 2,299 lines.
