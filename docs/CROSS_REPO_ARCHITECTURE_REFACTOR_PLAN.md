...
- Latest verified batch: OSS local remote-machine record/registry helpers are responsibility-owned; focused kernel provider-request tests pass.
- Latest verified batch: OSS relay registry owns peer maps, daemon presence projection, aliasing, pending requests, and subscriptions; relay tests pass.
...
- Latest verified batch: OSS workflow definition topology mutation and canvas reconciliation are responsibility-owned; focused session tests pass.
- Latest verified batch: OSS workflow scheduling defaults, queued launches, and watchdog definitions are responsibility-owned; focused session tests pass.
- Latest verified batch: OSS workflow failure/console diagnostics are responsibility-owned; focused session tests pass.
- Latest verified batch: OSS workflow output payloads, submissions, artifacts, completions, and handoff payloads are responsibility-owned; focused session tests pass.
- Latest verified batch: OSS workflow turn/status envelope and runtime tool-call state are responsibility-owned; focused session tests pass.
- Latest verified batch: OSS workflow messages and node-run records are responsibility-owned; focused session tests pass.
- Latest verified batch: OSS workflow-run aggregate state is responsibility-owned; focused session tests pass.
- Latest verified batch: OSS runtime interaction contracts are responsibility-owned; focused session tests pass.
- Latest verified batch: OSS prompt queue contracts are responsibility-owned; focused session tests pass.
- Latest verified batch: OSS session identity, invite, and creation contracts are responsibility-owned; focused session tests pass.
...
- Latest verified batch: Codex JSON-RPC message contracts are responsibility-owned; focused Codex client tests pass.
- Latest verified batch: Codex endpoint health probing is responsibility-owned; focused Codex client tests pass.
- Latest verified batch: Codex socket type and timeout mechanics are responsibility-owned; focused Codex client tests pass.
- Latest verified batch: Codex server-request approval and runtime-tool responses are responsibility-owned; focused Codex client tests pass.
- Latest verified batch: Codex JSON-RPC connect/request/notification transport is responsibility-owned; focused Codex client tests pass.
- Latest verified batch: Codex thread/turn runtime request construction and config override wiring are responsibility-owned; focused Codex client tests pass.
...
- Latest verified batch: OpenCode message/part/token contracts are responsibility-owned; focused OpenCode client/runtime tests pass.
- Latest verified batch: OpenCode session endpoint operations are responsibility-owned; focused OpenCode client tests pass.
- Latest verified batch: OpenCode catalog, health, and MCP endpoint operations are responsibility-owned; focused OpenCode client tests pass.
- Latest verified batch: OpenCode SSE event contracts live with event parsing; focused OpenCode client tests pass.
...
- Latest verified batch: Codex launch MCP/env/model-catalog config is responsibility-owned; focused Codex provider tests pass.
- Latest verified batch: Codex catalog endpoint startup and port selection policy are responsibility-owned; focused Codex provider tests pass.
...
- Latest verified batch: Claude runtime prompt/attachment input projection is responsibility-owned; focused Claude runtime tests pass.
- Latest verified batch: Claude runtime state/binding contracts are responsibility-owned; focused Claude runtime tests pass.
- Latest verified batch: Claude runtime JSON event projection is responsibility-owned; focused Claude runtime tests pass.
...
- Latest verified batch: Codex runtime event draining and quiet-terminal finalization are responsibility-owned; focused Codex runtime tests pass.
