...
- Latest verified batch: shared slice lifecycle, display endpoint, and provider-auth import request builders moved into `packages/kernel-client/src/ipc-slice-requests.ts`, with `ipc-requests.ts` retained as a compatibility barrel.
