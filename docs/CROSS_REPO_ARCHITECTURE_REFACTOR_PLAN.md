...
- Latest verified batch: MCP/skill capability shell commands moved into `packages/kernel-client/src/shell-capability-command.ts`; agent reference resolution moved into `shell-agent-resolver.ts`; `shell-executor.ts` is down to 1,679 lines.
- Latest gates for owned files: `pnpm --filter @arroba/kernel-client run test` and scoped diff check pass. Full Cloud API test is still blocked by dirty Cloud web client changes expecting `terminal-runtime.js`; full kernel `cargo fmt --check` is blocked by unrelated formatting drift outside these slices.
