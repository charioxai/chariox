...
- Latest verified batch: shared remote machine, waiting-room, pairing, terminal, and paired-client request builders moved into `packages/kernel-client/src/ipc-remote-connection-requests.ts`, with `ipc-requests.ts` retained as a compatibility barrel.
