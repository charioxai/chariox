...
2026-05-18: Latest verified stage is Cloud web. Terminal kernel events, session snapshots, prompt submit/stop, production start/join, history hydration, output reconciliation, remote/dev startup, prompt DOM, workspace panel DOM, waiting-room session DOM, create-session DOM, sidebar agent config DOM, and terminal runtime interaction DOM routing now live in focused controllers with tests/source guards. `client.ts` is 6,912 lines; next highest-impact slice is freeform agent dialog DOM routing.
