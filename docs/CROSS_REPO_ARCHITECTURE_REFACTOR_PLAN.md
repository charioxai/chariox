...
- Latest verified batch: provider run actor tests moved to the modules that own them: enqueue/stop tests in `run_actor/command_enqueue.rs`, runtime tombstone tests in `run_actor/runtime_slots.rs`; `run_actor.rs` is now a 139-line mailbox root.
- Latest gates for owned files: `cargo test --manifest-path apps/kernel/Cargo.toml provider::run_actor --lib -- --test-threads=1`, scoped rustfmt check, and scoped diff check pass. Full Cloud API test is currently blocked by dirty Cloud web client changes expecting `terminal-runtime.js`; full kernel `cargo fmt --check` is blocked by unrelated formatting drift outside these slices.
