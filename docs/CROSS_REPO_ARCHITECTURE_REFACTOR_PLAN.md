...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh policy, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal records/lifecycle, terminal transport lifecycle/status/target projection, kernel-directory projection policy, waiting-room session lifecycle/menu policy, freeform dialog/session-label/prompt-sizing/kernel-row/template/turn-clock state, freeform agent projection, agent substitute snapshot parsing, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,792 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands, agent utilities, remote relay inventory, capability dispatch, provider process control, waiting-room control, session read projection/inspection, session membership authorization, terminal output execution/projection, provider launch pending tracking, provider visibility policy, response redaction, and many router executors are responsibility modules. `runtime/router.rs` is still the main command router and is 8,439 lines.
- Latest verified slices: OSS required capability dispatch moved to `capability_executor.rs`; Cloud freeform turn-clock policy moved to `terminal/freeform-turn-clock-state.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test` (620 tests), `pnpm -r --if-present lint`, and `git diff --check`.
