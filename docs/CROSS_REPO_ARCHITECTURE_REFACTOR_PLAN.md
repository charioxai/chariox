...
- Cloud API boundary split is in place: `server.ts` is route composition; account control, admin, billing, browser dashboard/cache, pairing/revocation, relay presence, machine runtime profiles, relay-kernel bootstrap, browser cloud sessions/logout, device login, managed history, session invites, and runtime-token issuance delegate to focused service use cases; route/helper/contract files are domain-owned.
...
- OSS responsibility modules now cover kernel transport, router factory/composition/store+actor wiring, command admission/refresh, pre-lane dispatch, refresh-tracked dispatch, priority fallback dispatch, session actor focus/lane/execution/store/projection policy, capability runtime dispatch, credential/slice/workflow-forwarding runtime tool adapters, authenticated workflow runtime tool dispatch, local and remote managed-I/O dispatch, managed-I/O access/permission policy, capability registry packaging, remote capability sync, relay/cloud/runtime/status bridges, caller identity/redaction, membership authorization, remote authorization/provider projection/terminal output/session history/session read/session lifecycle/status/session actor projection test coverage, runtime projections, provider/agent/session/workspace/history/capability/waiting-room execution, launch tracking, response refresh, runtime lane cleanup, visibility policy, response redaction, and active drill naming cleanup. `runtime/router.rs` is now a small router/test-support module at 854 lines.
- Latest verified batch: OSS session actor projection router tests moved to `apps/kernel/src/runtime/router/tests/session_actor_projection.rs`.
- Latest gates for owned files: rustfmt check on owned router files, focused session actor/router tests, and diff check. Full kernel library tests are currently blocked by reproducible `local_request_api_invokes_lists_gets_and_cancels_workflow_runs` failure outside this diff; full kernel `cargo fmt --check` is also blocked by unrelated existing formatting drift outside this slice.
