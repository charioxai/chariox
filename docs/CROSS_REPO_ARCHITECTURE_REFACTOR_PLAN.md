...
- Cloud API boundary split is in place: `server.ts` is route composition; relay-kernel bootstrap, browser cloud sessions, device login, and runtime-token issuance delegate to focused service use cases; route/helper/contract files are domain-owned.
...
- Latest verified batch: Cloud device login use cases moved to `apps/api/src/device-login-service.ts`.
