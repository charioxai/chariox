...
- OSS responsibility modules now cover kernel transport, router factory/composition/store+actor wiring, command caller identity and local request metadata, cloud relay profile store/access policy, cloud relay login/pairing/connection, cloud session control, session snapshot projection, daemon health projection models, projection test coverage by owner module, state workflow dispatch aggregation, workflow request routing, workflow launch/resume owned transitions, workflow completion/failure owned transitions, workflow publication/pairing admin, workflow definition/topology admin, prompt activity/history side effects, remote prompt ownership, remote prompt dispatch transport, prompt git turn observation, structured provider-output polling, agent prompt target resolution, provider launch request preparation, provider MCP continuation, provider launch lifecycle, provider prompt settlement/failure, provider output pumping, managed-I/O workspace context, pending runtime interaction/reload/continuation state, runtime interaction lifecycle, runtime interaction owned transitions, terminal runtime state, session lookup state, runtime state views/health, agent/session config mutation, agent config owned mutations, agent lifecycle owned transitions, attachment owned transitions, capability owned context, session lifecycle orchestration, command admission/refresh, pre-lane dispatch, refresh-tracked dispatch, priority fallback dispatch, session actor focus/lane/execution/store/projection policy/test coverage, agent inline attachment materialization and actor test coverage, capability runtime dispatch, credential/slice/workflow-forwarding runtime tool adapters, authenticated workflow runtime tool dispatch, local and remote managed-I/O dispatch, managed-I/O access/permission policy, capability registry packaging, remote capability sync, relay/cloud/runtime/status bridges, caller identity/redaction, membership authorization, router test coverage by responsibility, runtime projections, provider/agent/session/workspace/history/capability/waiting-room execution, launch tracking, response refresh, runtime lane cleanup, visibility policy, response redaction, and active drill naming cleanup. `runtime/router.rs` is 207 lines; `runtime/session_actor.rs` is 136 lines; `runtime/agent_actor.rs` is 591 lines; `runtime/agent_actor/agent_resolution.rs` is 167 lines; `runtime/projection.rs` is 52 lines; `runtime/state/mod.rs` is 245 lines; `runtime/state/session.rs` is 313 lines; `runtime/state/prompt.rs` is 693 lines; `runtime/state/prompt_activity_owned_state.rs` is 436 lines; `runtime/state/remote_prompt_owned_state.rs` is 208 lines; `runtime/state/prompt_dispatch.rs` is 835 lines; `runtime/state/remote_prompt_dispatch_runtime.rs` is 309 lines; `runtime/state/prompt_git_observer_runtime.rs` is 122 lines; `runtime/state/workflow_admin.rs` is 417 lines; `runtime/state/workflow_definition_owned_state.rs` is 595 lines; `runtime/state/workflow_publication_owned_state.rs` is 361 lines; `runtime/state/workflow_dispatch.rs` is 566 lines; `runtime/state/workflow_launch_owned_state.rs` is 350 lines; `runtime/state/workflow_completion_owned_state.rs` is 457 lines; `runtime/state/provider_output_runtime.rs` is 707 lines; `runtime/state/structured_provider_output_runtime.rs` is 273 lines; `runtime/command.rs` is 393 lines; `runtime/cloud_relay_executor.rs` is 95 lines.
- Latest verified batch: OSS agent prompt target resolution extraction from `agent_actor.rs`.
- Latest gates for owned files: agent actor focused tests, rustfmt check on owned files, and diff check pass. Cloud API typecheck and focused API service tests pass; full Cloud API test is currently blocked by dirty Cloud web client changes expecting `terminal-runtime.js`. Full kernel library tests remain blocked by reproducible `local_request_api_invokes_lists_gets_and_cancels_workflow_runs`; full kernel `cargo fmt --check` is blocked by unrelated existing formatting drift outside these slices.
