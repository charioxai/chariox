...
- Latest verified batch: shared provider process, auth, catalog, provider-run selection, and provider-run launch request builders moved into `packages/kernel-client/src/ipc-provider-requests.ts`, with `ipc-requests.ts` retained as a compatibility barrel.
