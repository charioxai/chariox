...
- Latest verified batch: workspace, agent lifecycle/utility, remote-access, cloud-relay, config/capability, provider-control, history, terminal/interaction, capability, and slice local-daemon wire contracts moved into domain modules under `local/api/types/`; `types.rs` remains the protocol compatibility barrel.
