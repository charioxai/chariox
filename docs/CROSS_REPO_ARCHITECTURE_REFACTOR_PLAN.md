...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh/background-render policy, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal records/lifecycle, terminal transport lifecycle/status/target projection, kernel-directory projection policy, waiting-room session lifecycle/menu policy, slash-command parsing, provider-run profile projection, freeform dialog/session-label/prompt-sizing/kernel-row/template/default-agent/pane-shell/live-desk/turn-clock/agent-reference/desk/config/command-center state, freeform footer config/update policy, freeform agent projection, agent substitute snapshot parsing, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,599 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands, daemon health projection, local caller identity projection, relay diagnostic projection, runtime MCP proxy dispatch, cloud relay connection refresh/request dispatch, relay peer lease/projection execution, workspace worktree command policy, agent controls, agent utilities, focus projection refresh, remote relay inventory, capability dispatch, provider process control, waiting-room control, session read projection/inspection/collaboration, pairing dispatch, session membership authorization, terminal output execution/projection, provider launch pending tracking, provider visibility policy, response redaction, and many router executors are responsibility modules. `runtime/router.rs` is still the main command router and is 8,022 lines.
- Latest verified slices: OSS pairing dispatch moved to `pairing_invite_executor.rs`; Cloud waiting-room background render scheduling moved to `terminal/waiting-room-background-render.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- waiting-room-background-render` (645 tests), `pnpm -r --if-present lint`, and `git diff --check`.
