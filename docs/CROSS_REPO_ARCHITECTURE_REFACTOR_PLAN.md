...
- Cloud API boundary split is in place: `server.ts` is route composition; billing, pairing/revocation, relay-kernel bootstrap, browser cloud sessions, device login, managed history, session invites, and runtime-token issuance delegate to focused service use cases; route/helper/contract files are domain-owned.
...
- Latest verified batch: Cloud pairing and revocation use cases moved to `apps/api/src/pairing-service.ts`.
