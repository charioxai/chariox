...
- Remote managed-I/O dispatch now separates composition, outbound leased-worker forwarding, and home-kernel forwarded execution. Remaining OSS hotspots: `runtime/state/tool_dispatch/managed_io_remote_dispatch/forwarded.rs` 432 lines, `runtime/history_requests.rs` 497 lines, and `runtime/workspace_repo_files.rs` 461 lines.
- Latest verified batch: remote managed-I/O outbound forwarding and forwarded home-kernel dispatch moved into `managed_io_remote_dispatch/outbound.rs` and `managed_io_remote_dispatch/forwarded.rs`.
- Latest gates for owned files: `cargo test --manifest-path apps/kernel/Cargo.toml managed_io_remote --lib -- --test-threads=1`, `cargo test --manifest-path apps/kernel/Cargo.toml managed_io --lib -- --test-threads=1`, rustfmt check on owned files, and diff check pass. Cloud API typecheck and focused API service tests pass; full Cloud API test is currently blocked by dirty Cloud web client changes expecting `terminal-runtime.js`. Full kernel `cargo fmt --check` is blocked by unrelated existing formatting drift outside these slices.
