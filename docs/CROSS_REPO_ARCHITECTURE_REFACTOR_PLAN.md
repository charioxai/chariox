...
2026-05-18: Latest verified stage is Cloud web. Freeform dialog orchestration, waiting-room key handling, terminal kernel event routing, session snapshot reconciliation, prompt submit/stop, production session start/join, history hydration, live terminal output pane reconciliation, remote/dev terminal startup, and prompt/attachment DOM orchestration now live in focused controllers with source guards. `client.ts` is 8,033 lines; next highest-impact slice is remaining waiting-room/workspace global DOM routing.
