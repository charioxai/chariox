...
- 2026-05-13: Cloud browser relay kernel bootstrap ownership cutover started in `arroba-cloud`. `/browser/relay-kernel/bootstrap` now delegates target selection, relay URL validation, relay token minting, and cached waiting-room snapshot lookup to `CloudApiService.bootstrapBrowserRelayKernel`; target selection/relay URL validation live in a focused `browser-relay-target-selection` module; the bootstrap route registration moved into `routes/browser-relay-kernel.ts`; API architecture tests guard against active `/web-cli` routes and direct bootstrap token minting in the route module.
