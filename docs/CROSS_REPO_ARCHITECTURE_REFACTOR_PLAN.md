...
- Cloud API boundary split is in place: `server.ts` is route composition; billing, browser dashboard/cache, pairing/revocation, relay-kernel bootstrap, browser cloud sessions, device login, managed history, session invites, and runtime-token issuance delegate to focused service use cases; route/helper/contract files are domain-owned.
...
- Latest verified batch: Cloud browser dashboard and waiting-room cache scoping moved to `apps/api/src/browser-dashboard-service.ts`.
