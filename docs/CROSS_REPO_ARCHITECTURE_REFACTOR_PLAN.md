...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh/background-render/modal-layer/workspace-suggestion/session-action-menu/workspace-search/worktree-refresh/create-alias/session-alias-edit policy, app-sidebar DOM projection, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal lifecycle/transport/status/target projection, kernel-directory projection policy, provider-run profile projection, freeform state/projection/session-alias policy, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,435 lines.
...
- Latest verified slices: OSS projected session-read fast path moved into `runtime/session_read_control.rs`; Cloud freeform session alias policy moved to `terminal/freeform-session-label-state.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- freeform-session-label-state` (676 tests).
