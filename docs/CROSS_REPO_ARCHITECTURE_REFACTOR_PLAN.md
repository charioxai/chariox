...
- Cloud web responsibility modules now cover browser kernel transport, waiting-room state/projection/rendering/connection display/refresh failures/refresh scheduling/polling, app sidebar/workflow/history/prompt/output/workspace/capabilities, terminal lifecycle/transport/target/launch/provider profile, freeform policy, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,215 lines.
...
- Latest verified batch: OSS post-command response refresh moved to `runtime/command_response_refresh.rs`; Cloud waiting-room polling moved into the refresh scheduler.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- waiting-room-refresh-scheduler` (737 tests).
