...
- Next targeted OSS hotspots: `runtime/state/tool_dispatch/managed_io_remote_dispatch.rs` 504 lines, `runtime/history_requests.rs` 497 lines, and `runtime/workspace_repo_files.rs` 461 lines.
- Latest verified batch: capability executor runtime context and transfer artifact/history handling moved into `runtime/capability_executor/context.rs` and `runtime/capability_executor/transfer.rs`.
