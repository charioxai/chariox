...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh/background-render/modal-layer/workspace-suggestion/session-action-menu/workspace-search/worktree-refresh/create-alias/session-alias-edit/agent-alias-edit policy, app-sidebar DOM projection, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal lifecycle/transport/status/target projection, kernel-directory projection policy, provider-run profile projection, freeform state/projection/session-alias policy, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,419 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands, daemon health projection, local caller identity projection, relay diagnostics, runtime MCP proxy dispatch, cloud relay dispatch, relay peer lease/projection execution, workspace dispatch/worktree policy, history dispatch, provider auth/catalog/process/run dispatch, agent control dispatch, agent utilities, focus projection refresh, remote inventory/registry dispatch, capability dispatch, waiting-room control, session read/projection/inspection/collaboration, pairing/user-config/credential/slice dispatch, membership authorization, terminal output execution/projection, provider launch tracking, provider visibility policy, and response redaction are responsibility modules. `runtime/router.rs` is still the main command router and is 7,685 lines.
- Latest verified slices: OSS remote relay inventory dispatch moved into `runtime/remote_relay_inventory.rs`; Cloud sidebar agent alias policy moved to `terminal/waiting-room-agent-alias-edit-state.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- waiting-room-agent-alias-edit-state` (682 tests).
