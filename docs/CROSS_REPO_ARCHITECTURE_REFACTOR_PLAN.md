...
- Latest verified batch: OSS router normal/background dispatch now uses shared workflow and interactive predicates instead of duplicating request membership in `priority_dispatch.rs`.
- Latest gates for owned files: Cloud API build, focused API tests, kernel-client tests, focused router test, file-level rustfmt, and scoped diff checks pass. Full Cloud API test is blocked by dirty Cloud web client changes; full kernel test currently fails in dirty slice/private-relay tests outside this slice.
