...
- Latest verified slices: OSS relay status/config dispatch moved into `runtime/relay_config_control.rs`; Cloud sidebar agent option-picker projection moved to `terminal/sidebar-agent-option-picker-projection.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- sidebar-agent-option-picker-projection sidebar-agent-config` (708 tests).
