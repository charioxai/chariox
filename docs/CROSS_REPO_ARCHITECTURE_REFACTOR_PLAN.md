...
- Latest verified batch: workspace, agent-utility, remote-access, cloud-relay, config/capability, and provider-control local-daemon wire contracts moved into domain modules under `local/api/types/`; `types.rs` remains the protocol compatibility barrel.
