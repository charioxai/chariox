...
- Latest verified batch: Cloud API body parsing, error/not-found handling, and operational routes moved into focused HTTP/route modules; `server.ts` is down to 177 lines.
- Latest gates for owned files: Cloud API build, focused API tests, kernel-client tests, and scoped diff checks pass. Full Cloud API test is still blocked by dirty Cloud web client changes expecting `terminal-runtime.js`; full kernel `cargo fmt --check` is blocked by unrelated formatting drift outside these slices.
