...
- Latest verified batch: shared agent spawn, alias, profile/config, substitute, focus, remote move, destroy, and list request builders moved into `packages/kernel-client/src/ipc-agent-requests.ts`, with `ipc-requests.ts` retained as a compatibility barrel.
