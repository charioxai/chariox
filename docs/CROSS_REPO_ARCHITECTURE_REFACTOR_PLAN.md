...
- Latest verified batch: workflow, waiting-room, session/prompt-control, workspace, agent lifecycle/utility, remote-access, cloud-relay, config/capability, provider-control, history, terminal/interaction, capability, and slice local-daemon wire contracts moved into domain modules under `local/api/types/`; `types.rs` remains the protocol compatibility barrel.
- Latest gates for owned files: `cargo test --manifest-path apps/kernel/Cargo.toml local::api --lib -- --test-threads=1`, scoped rustfmt check, and scoped diff check pass. Full Cloud API test is currently blocked by dirty Cloud web client changes expecting `terminal-runtime.js`; full kernel `cargo fmt --check` is blocked by unrelated formatting drift outside these slices.
