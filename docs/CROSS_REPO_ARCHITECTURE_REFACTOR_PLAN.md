...
- Latest verified batch: shared relay status/configuration, Cloud relay login/token, Cloud session invite, and collaborator request builders moved into `packages/kernel-client/src/ipc-relay-control-requests.ts`, with `ipc-requests.ts` retained as a compatibility barrel.
