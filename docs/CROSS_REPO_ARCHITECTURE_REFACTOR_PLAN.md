...
Cloud web remains the highest-impact area. Latest verified web stage: app-sidebar input focus, terminal session record ownership/background refresh, and waiting-room new-session launch orchestration moved out of `client.ts` into focused terminal modules with tests. `client.ts` is still very large at 9,753 lines, so subsequent web slices should keep targeting it before lower-impact work.
