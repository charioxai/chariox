...
- Latest verified batch: Cloud API health/readiness/metrics moved into `cloud-api-operational-service.ts`; `cloud-api-service.ts` is now 496 lines of service composition.
- Latest gates for owned files: `pnpm --filter @arroba-cloud/api run build`, focused API service tests, and scoped diff check pass. Full Cloud API test is still blocked by dirty Cloud web client changes expecting `terminal-runtime.js`; full kernel `cargo fmt --check` is blocked by unrelated formatting drift outside these slices.
