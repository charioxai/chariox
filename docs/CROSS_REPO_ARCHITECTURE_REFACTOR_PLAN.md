...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh/background-render/modal-layer/workspace-suggestion/session-action-menu/workspace-search/worktree-refresh/create-alias/session-alias-edit/agent-alias-edit/agent-workspace-edit/agent-option-picker state/projection, app-sidebar DOM projection, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal lifecycle/transport/status/target projection, terminal launch request policy, kernel-directory projection policy, provider-run profile application/projection, freeform state/projection/session-alias/dialog-init/kernel-request policy, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,292 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands/dispatch, interactive command dispatch, daemon health projection/dispatch, local caller identity projection, relay diagnostics/config dispatch, runtime MCP proxy dispatch, cloud relay dispatch, relay peer lease/projection execution, workspace dispatch/worktree policy, history dispatch, provider auth/catalog/process/run dispatch, agent control/utility dispatch, focus projection refresh, remote inventory/registry dispatch, capability dispatch, waiting-room control/dispatch, session read/projection/inspection/collaboration, pairing/user-config/credential/slice dispatch, membership authorization, terminal output execution/projection, provider launch tracking, provider visibility policy, and response redaction are responsibility modules. `runtime/router.rs` is still the main command router and is 7,632 lines.
- Latest verified slices: OSS interactive dispatch moved into `runtime/interactive_command_dispatcher.rs`; Cloud provider-run profile application moved to `terminal/provider-run-profile.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- provider-run-profile` (721 tests).
