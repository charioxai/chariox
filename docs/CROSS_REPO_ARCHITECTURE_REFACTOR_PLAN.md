...
- Remote managed-I/O dispatch now separates composition, outbound leased-worker forwarding, home-kernel admission/routing, forwarded reads, forwarded text edit/write, and forwarded patch/delete/move mutations. Remaining OSS hotspots: `runtime/history_requests.rs` 497 lines and `runtime/workspace_repo_files.rs` 461 lines.
- Latest verified batch: forwarded managed-I/O read/text/mutation handlers moved under `managed_io_remote_dispatch/forwarded/`.
