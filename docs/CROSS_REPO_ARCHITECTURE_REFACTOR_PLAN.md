...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh view policy, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal records/lifecycle, terminal transport lifecycle/status/target projection, kernel-directory projection policy, waiting-room session lifecycle/menu policy, slash-command parsing, provider-run profile projection, freeform dialog/session-label/prompt-sizing/kernel-row/template/default-agent/pane-shell/live-desk/turn-clock/agent-reference/desk/config/command-center state, freeform footer config/update policy, freeform agent projection, agent substitute snapshot parsing, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,648 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands, daemon health projection, local caller identity projection, relay diagnostic projection, runtime MCP proxy dispatch, cloud relay connection refresh/request dispatch, relay peer lease/projection execution, workspace worktree command policy, agent controls, agent utilities, focus projection refresh, remote relay inventory, capability dispatch, provider process control, waiting-room control, session read projection/inspection/collaboration, session membership authorization, terminal output execution/projection, provider launch pending tracking, provider visibility policy, response redaction, and many router executors are responsibility modules. `runtime/router.rs` is still the main command router and is 8,059 lines.
- Latest verified slices: OSS session collaboration dispatch moved to `session_collaboration_executor.rs`; Cloud waiting-room refresh view projection moved to `terminal/waiting-room-refresh-state.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- waiting-room-refresh-state` (642 tests), `pnpm -r --if-present lint`, and `git diff --check`.
