...
- Latest verified batch: shared MCP server, skill, and agent capability request builders moved into `packages/kernel-client/src/ipc-extension-requests.ts`, with `ipc-requests.ts` retained as a compatibility barrel.
