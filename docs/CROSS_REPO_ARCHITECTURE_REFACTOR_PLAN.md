...
- Latest verified batch: shared session transcript, prompt input, archive query/search, and semantic history request builders moved into `packages/kernel-client/src/ipc-history-requests.ts`, with `ipc-requests.ts` retained as a compatibility barrel.
