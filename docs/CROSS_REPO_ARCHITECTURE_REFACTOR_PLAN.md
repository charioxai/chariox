...
- OSS modules now cover router composition/actors, command metadata/admission/dispatch, relay/cloud/session bridges, projections, runtime state views, prompt/workflow/provider/session/agent/capability/attachment ownership, local prompt admission/dispatch, prompt queue advancement, prompt skill context, provider focus sync, managed I/O, runtime interactions, remote prompt submission/lifecycle, workflow endpoints/settings/topology/turn prompts, tool dispatch, response refresh/redaction, agent lane command execution, and responsibility-owned tests. Current hotspots: none over 600 lines in `runtime/state` or `agent_actor.rs`. Reduced coordinators: `runtime/router.rs` 207 lines, `runtime/session_actor.rs` 136 lines, `runtime/agent_actor.rs` 419 lines, `runtime/state/prompt_dispatch.rs` 311 lines, `runtime/state/prompt.rs` 439 lines, `runtime/state/provider.rs` 315 lines, `runtime/state/workflow_dispatch.rs` 337 lines, `runtime/state/remote_prompt_dispatch_runtime.rs` 309 lines, `runtime/state/workflow_definition_owned_state.rs` 399 lines, `runtime/state/provider_output_runtime.rs` 230 lines, `runtime/state/mod.rs` 256 lines.
- Latest verified batch: OSS agent lane command execution moved from `agent_actor.rs` into `agent_actor/command_executor.rs`.
