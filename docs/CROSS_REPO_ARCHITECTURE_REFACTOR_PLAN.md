...
- Latest verified batch: shell placement parsing and local git worktree preparation moved into `packages/kernel-client/src/shell-placement.ts`; `shell-executor.ts` is down to 3,499 lines.
- Latest gates for owned files: `pnpm --filter @arroba/kernel-client run test` passes with local socket permissions; scoped diff check passes. Full Cloud API test is still blocked by dirty Cloud web client changes expecting `terminal-runtime.js`; full kernel `cargo fmt --check` is blocked by unrelated formatting drift outside these slices.
