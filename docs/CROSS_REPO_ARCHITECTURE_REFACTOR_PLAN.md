...
- OSS modules now cover router composition/actors, command metadata/admission/dispatch, relay/cloud/session bridges, projections, runtime state views, prompt/workflow/provider/session/agent/capability/attachment ownership, local prompt admission, prompt queue advancement, prompt skill context, provider focus sync, managed I/O, runtime interactions, remote prompt submission/lifecycle, workflow endpoints/settings/topology/turn prompts, tool dispatch, response refresh/redaction, and responsibility-owned tests. Current hotspots: `runtime/agent_actor.rs` 591 lines. Reduced coordinators: `runtime/router.rs` 207 lines, `runtime/session_actor.rs` 136 lines, `runtime/state/prompt_dispatch.rs` 597 lines, `runtime/state/prompt.rs` 439 lines, `runtime/state/provider.rs` 315 lines, `runtime/state/workflow_dispatch.rs` 337 lines, `runtime/state/remote_prompt_dispatch_runtime.rs` 309 lines, `runtime/state/workflow_definition_owned_state.rs` 399 lines, `runtime/state/provider_output_runtime.rs` 230 lines, `runtime/state/mod.rs` 255 lines.
- Latest verified batch: OSS provider focus synchronization and prompt queue advancement moved out of `provider.rs`.
