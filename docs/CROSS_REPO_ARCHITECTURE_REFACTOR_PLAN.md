...
- Cloud web has responsibility modules for browser kernel transport, waiting-room projection/state/refresh/background-render/modal-layer/workspace-suggestion/session-action-menu/workspace-search/worktree-refresh/create-alias/session-alias-edit/agent-alias-edit/agent-workspace-edit/agent-option-picker state/projection, app-sidebar DOM projection, workflow route state, history, prompt, output, sidebar, workspace, capabilities, terminal lifecycle/transport/status/target/refresh policy, terminal launch request policy, kernel-directory projection policy, provider-run profile application/projection, freeform state/projection/session-alias/dialog-init/kernel-request policy, and session projection fingerprinting. `client.ts` is still the main coordinator and is 10,286 lines.
- OSS protocol/client split is started; kernel transport frames/events, kernel lifecycle commands/dispatch, interactive command dispatch, daemon health projection/dispatch, shared caller identity policy, relay diagnostics/config dispatch, runtime MCP proxy dispatch, cloud relay dispatch, relay peer lease/projection execution, workspace dispatch/worktree policy, history dispatch, provider auth/catalog/process/run dispatch, agent control/utility dispatch, focus projection refresh, remote inventory/registry dispatch, capability dispatch, waiting-room control/dispatch, session read/projection/inspection/collaboration, pairing/user-config/credential/slice dispatch, membership authorization, terminal output execution/projection, provider launch tracking, provider visibility policy, and response redaction are responsibility modules. `runtime/router.rs` is still the main command router and is 7,632 lines.
- Latest verified slices: OSS session collaboration now uses shared caller identity policy; Cloud terminal refresh scheduling and working-agent policy moved to `terminal/session-lifecycle.ts`.
- Latest focused gates passed: `cargo test --manifest-path apps/kernel/Cargo.toml --lib -- --test-threads=1` (730 tests), `pnpm --filter @arroba-cloud/web test -- session-lifecycle` (723 tests).
