...
- Latest verified batch: relay socket connection lifecycle moved into `transport/relay_client/connector.rs`; `relay_client.rs` is 2,705 lines and now mostly tests/module wiring.
- Latest gates for owned files: `cargo test --manifest-path apps/kernel/Cargo.toml relay_client --lib -- --test-threads=1`, rustfmt check on owned files, and scoped diff check pass. Cloud API typecheck and focused API service tests pass; full Cloud API test is currently blocked by dirty Cloud web client changes expecting `terminal-runtime.js`. Full kernel `cargo fmt --check` is blocked by unrelated existing formatting drift outside these slices.
