...
- Latest verified batch: shared terminal I/O, prompt lifecycle, transfer/screenshot, runtime notice, directory tree, and native interaction request builders moved into `packages/kernel-client/src/ipc-terminal-runtime-requests.ts`, with `ipc-requests.ts` retained as a compatibility barrel.
