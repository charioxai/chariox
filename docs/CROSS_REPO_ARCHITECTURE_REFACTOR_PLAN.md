...
- Latest verified batch: OSS live cloud relay drill no longer depends on deleted `/web-cli` inventory streams.
- Latest gates for owned files: script syntax check, stale-name scan for the edited drill, and diff check. Full OSS fmt is blocked by unrelated native-provider protocol formatting drift.
