...
| `/config workspace-live-sync` / `config workspace-live-sync` | Yes | Uses the dedicated `SetWorkspaceLiveSyncMode` kernel API for global `providers.workspace_live_sync`; terminal and shell clients render mutation effects returned by the kernel. | Keep as the friendly wrapper over the dedicated workspace live sync mode path. | Yes, implemented through central provider reload policy. |
