...
- Runtime MCP exposes capability control-plane tools `arroba.list_capabilities` / `list_capabilities` and `arroba.request_capability` / `request_capability`. These list Arroba-managed MCPs and skills visible to the current workspace and let the current agent request one by `kind` and `name`. V1 auto-grants valid requests; newly requested MCPs become usable on the next provider launch, while newly requested skills are effective immediately because the request returns the full `SKILL.md` body by default. Remote worker agents forward these capability calls to the home kernel; skill grants transfer a hash-verified package of the home skill directory and materialize it in the worker workspace under `.arroba/remote/skills/...`. Remote prompt dispatch verifies/synchronizes granted skills before submit and injects worker-local `materialized_root` paths into the prompt context.
