...
`DestroyExecutionLease` also cleans up agents still owned by that lease through the existing leased-agent deletion path before removing the lease. Other leases may share the hidden worker Room and must remain usable. The real-relay regression launches three synthetic managed provider processes across two leases in one backing Room, releases the two-agent lease, verifies its processes stop, and sends input to the surviving lease's provider before final cleanup. If one agent teardown fails, the worker still attempts every other agent owned by the lease, retains the first error and the incomplete lease, and removes neither the lease nor its completion receipt. A retry enumerates only the remaining agents and converges on the normal idempotent completion state. A focused failure-injection test covers the partial-failure and retry path. This is local worker lifecycle evidence, not validation of real provider execution or worker-restart reconciliation.
