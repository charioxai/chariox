...
- Runtime MCP exposes capability control-plane tools `arroba.list_capabilities` / `list_capabilities` and `arroba.request_capability` / `request_capability`. These list Arroba-managed MCPs and skills visible to the current workspace and let the current agent request one by `kind` and `name`. V1 auto-grants valid requests; newly requested MCPs become usable on the next provider launch, while newly requested skills affect the next prompt.
