...
- A Selkies endpoint advertises `view`, `websocket`, `h264`, and `software_encoding`. It does not grant keyboard, pointer, clipboard, or resize authority. Those remain kernel-owned Environment operations. Its local URL identifies the private slice process, not a publicly usable viewer link. Selkies cannot use the legacy raw HTTP display tunnel; encrypted runtime transport and scoped viewer admission must be wired before enabling it in released clients. Clients that select or interpret this backend require v280; clients that do not use it keep their existing minimum version.
- Protocol v281 defines encrypted display fragments with `protocol: "chariox-display-v1"`, a connection-specific `stream_id`, sender direction (`kernel` or `viewer`), a zero-based per-direction `sequence`, message `kind` (`text` or `binary`), `final_fragment`, and `data_base64`. The entire fragment is encrypted with the existing relay payload encryption to the admitted peer key. Each fragment carries at most 64 KiB of raw data, and a reassembled message is at most 4 MiB. Wrong peer, stream, direction, version, replay, out-of-order, malformed, and oversized packets permanently close that channel. Reconnect requires a fresh stream identity. This layer does not grant admission or input authority, has no history persistence, and does not yet enable Selkies viewing. Runtime routing must add expiring kernel-owned viewer admission and bounded queues before release; do not pass plaintext Selkies bytes into the legacy tunnel. Clients using these encrypted fragments require v281.
...
`DestroyExecutionLease` also cleans up agents still owned by that lease through the existing leased-agent deletion path before removing the lease. Other leases may share the hidden worker Room and must remain usable. The real-relay regression launches three synthetic managed provider processes across two leases in one backing Room, releases the two-agent lease, verifies its processes stop, and sends input to the surviving lease's provider before final cleanup. Individual leased-agent deletion retains its authoritative record and withholds its completion receipt until provider termination and tracking, attachment removal, backing-agent removal, and last-user session cleanup succeed. Already-removed resources count as progress on a retry. Production cleanup errors propagate to lease deletion; the worker still attempts every other agent owned by the lease, retains the first error and the incomplete lease, and removes neither the lease nor its completion receipt. A retry enumerates only fully cleaned agents as absent and converges on the normal idempotent completion state. Focused failure-injection tests cover both the leased-agent retention boundary and lease-level partial failure. This is local worker lifecycle evidence, not validation of real provider execution or worker-restart reconciliation.
