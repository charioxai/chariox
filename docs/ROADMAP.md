...
- in M4.5 cleanup as of 2026-04-15
...
- the seven production ownership points are closed: direct-cutover baseline, session ownership, prompt ownership, provider process/output ownership, workflow/runtime-tool ownership, transport/relay ownership, and runtime fallback deletion now use owned runtime paths. Remaining M4.5 work is the dead-code purge of unused app-backed helper surfaces plus final docs/invariant alignment; workspace coordination should stay at its current coarse claim scope until the final I/O-coordination slice
