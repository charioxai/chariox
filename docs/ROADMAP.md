...
- first implementation slices are landed: kernel command/event envelopes, event replay gaps, projection metadata, command routing, bounded interactive routing, typed CLI replay-gap handling, command-id retry/fanout safety, inbound WebSocket admission bounds, provider-run actor runtime isolation, provider actor enqueue error propagation, reserved-listener websocket tests, `KernelSessionService` ownership for session lifecycle/focus/resize/end/delete operations, `KernelAgentService` ownership for prompt submit/cancel/complete/queue-advance lifecycle operations, per-agent prompt command mailboxes, per-session attach/focus/resize/end/delete command mailboxes with cleanup on session close, a focused-agent projection refreshed by session mailboxes and agent lifecycle responses for untargeted prompt routing, warmed projections for `GetSessionState`, `ListSessions`, `GetSessionHistory`, `GetProviderRun`, `ListProviderProcesses`, `GetProviderCatalog`, and agent/workflow inspection reads, prompt lifecycle publication into the shared session projection, one shared agent-runtime active/queued prompt projection read model, a dedicated flattened `PromptRuntimeState` session prompt-runtime module for compatibility prompt mutation/projections, daemon health projection snapshots for actor queues, provider runtime lanes, prompt counts, provider-catalog cache state, kernel websocket transport pressure, workspace worktree-collision state, `WorkspaceCoordinator` write-claim enforcement for explicit file-writing capabilities, provider prompt lifecycle worktree claims for cross-session same-worktree conflicts, and workflow node dispatch blocking/retry on workspace claims
