...
- M7: Arroba-owned MCP and skill management. In progress: MCP/skill registries, slash-command install/list/show/grant/revoke/grants, per-agent grants, local Codex/OpenCode MCP rendering, granted-skill summary injection, and local explicit full skill-body injection are landed; import/update/uninstall, runtime discovery/request tools, remote materialization, and drills remain open.
