...
Provider-facing mouse and keyboard tools use that route as well. A leased provider call reaches the home kernel first, where the home derives the agent Actor, validates the current Room Environment, admits one redacted Computer Action, and sends the physical input to the bound worker. The worker executes the input but does not create a private Room, choose an Actor, or record a second Action ledger. This is the same authority path used by browser tools, human Computer input, takeover cancellation, and Computer credential input.
