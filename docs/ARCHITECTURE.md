...
- `SessionStateProjectionStore` now serves warmed `GetSessionState` and `ListSessions` reads from a router-owned projection. The router refreshes it from responses that already carry session/list snapshots and from transitional post-mutation snapshots for commands that change session state without returning a full session.
...
- `DaemonApp` still remains the compatibility facade for many paths. The next major implementation step is to move the rest of `KernelSessionService` and `KernelAgentService` state into those mailbox owners, expand actor-owned projections beyond focused-agent routing and warmed session/list/history snapshots, and make provider/prompt-state reads projection-first so request handlers no longer depend on `Arc<Mutex<DaemonApp>>` for hot-path work.
