...
- `WorkspaceCoordinator` now enforces scoped worktree write claims for explicit file-writing capabilities (`EditFile` and `StoreTransferredFile`) and active local provider prompt lifecycles. Provider prompts acquire claims before dispatch, release them through prompt cleanup on completion/cancellation/dispatch failure/session cleanup, and reject cross-session same-worktree prompt conflicts. Workflow node dispatch claims, file-level scopes, and port claims are still follow-up work.
