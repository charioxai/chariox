...
- `AgentRuntimeProjectionStore` now materializes per-agent active/queued prompt read models from session projection refreshes and exposes direct agent/session-scoped reads for future actor-owned prompt inspection. Daemon health treats that agent-runtime projection as the canonical prompt-count source and mirrors the counts into legacy session-projection health fields for compatibility while prompt queue ownership moves out of compatibility session state.
- Compatibility session prompt mutation is now isolated in `PromptRuntimeState` in `session/prompt_runtime.rs`. It owns the per-agent active/queued prompt map plus the legacy session-level active prompt, queued prompt, and scheduler projections, and `RuntimeSession` only flattens/forwards it for compatibility while the next slice moves that owner behind `AgentRuntime`.
- The agent-runtime projection now carries the front queued prompt per agent, and local/remote queue advancement consults that projected candidate before falling back to compatibility session queue inspection.
...
- Provider idle settlement now checks projected per-agent active prompt state before falling back to compatibility session inspection when deciding whether to complete, finalize cancellation, or clear activity.
- `AgentRuntime` now carries the shared agent-runtime projection store into per-agent mailbox workers and publishes submit/cancel/complete prompt state from the mailbox execution path. The temporary private prompt-state shadow has been removed; `AgentRuntimeProjectionStore` is the single warm prompt-state read model while compatibility session state remains the mutation mirror.
