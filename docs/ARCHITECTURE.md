...
- Compatibility session prompt mutation is now isolated in `PromptRuntimeState`. It owns the per-agent active/queued prompt map plus the legacy session-level active prompt, queued prompt, and scheduler projections, and is flattened in serialization so existing daemon/API payloads stay stable while the next slice moves that owner behind `AgentRuntime`.
