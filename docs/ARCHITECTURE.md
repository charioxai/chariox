...
- `AgentRuntime` now carries the shared agent-runtime projection store into per-agent mailbox workers and publishes submit/cancel prompt state from the mailbox execution path. Router response refresh still keeps compatibility session projections current, but agent-runtime prompt read models are no longer only a router-side derivation for these commands.
