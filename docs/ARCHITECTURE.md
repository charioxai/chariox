...
- `PromptStateOwner` is now the kernel owner for per-agent active prompts and queued prompt backlogs. Prompt submit, complete, cancel, cancellation finalization, dispatch-failure cleanup, detach cleanup, workflow prompt submission, and local/remote queue advancement mutate that owner first and then mirror the resulting agent state into `RuntimeSession`.
...
- The agent-runtime projection now carries the front queued prompt per agent for warm routing previews, while the prompt owner remains authoritative when a projection is stale.
