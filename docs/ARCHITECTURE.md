...
- Relay-client daemon requests now normalize into `KernelCommandSource::RelayClient` commands and dispatch through `CommandRouter`, so proxied clients share actor admission, projection refresh, and overload behavior with local IPC/kernel transport instead of bypassing the runtime through `DaemonApp::handle_local_request`.
