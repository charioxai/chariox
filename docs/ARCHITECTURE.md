...
- `AgentRuntimeProjectionStore` now materializes per-agent active/queued prompt read models from session projection refreshes, giving daemon health an agent-runtime prompt-count view before prompt queue ownership fully moves out of compatibility session state.
