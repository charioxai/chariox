...
- `WorkspaceCoordinator` now enforces scoped worktree write claims for explicit file-writing capabilities (`EditFile` and `StoreTransferredFile`). Broader provider prompt lifecycle claims, workflow node dispatch claims, file-level scopes, and port claims are still follow-up work.
