...
- `SessionStateProjectionStore` now serves warmed `GetSessionState` reads from a router-owned projection. The router refreshes it from responses that already carry session snapshots and from transitional post-mutation snapshots for commands that change session state without returning a full session.
