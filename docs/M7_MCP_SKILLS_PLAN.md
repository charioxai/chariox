...
M7 adds Chariox-owned MCP and skill management. The design intentionally follows Codex's model where practical, while keeping Chariox responsible for orchestration, per-agent grants, local/remote placement, and provider-specific session rendering.

Current decision: the repo-local isolation spike in `docs/M7_MCP_ISOLATION_SPIKE_PLAN.md` validated the architecture. Production should now move from direct provider MCP config injection to Chariox-supervised provider-facing MCP proxies, while preserving provider-native MCP semantics for agents.
...
- M7.1/M7.2: Chariox-owned MCP config model and registry for project/user roots, including update and uninstall lifecycle operations.
...
- M7.4 partial: `/mcp import codex [name]`, `/mcp import opencode [name]`, and `/mcp import claude [name]` import supported provider MCP config entries into Chariox-owned MCP registry roots and report skipped entries.
...
- M7.6 partial: local Codex and OpenCode provider launches render only the target agent's granted Chariox MCPs into provider-native MCP config, while keeping Chariox runtime MCP separate. When the runtime MCP binding is available, granted third-party MCPs are rendered as provider-facing Chariox proxy entries instead of raw backing definitions.
- M7.7/M7.8: Codex-style `SKILL.md` metadata parsing and Chariox-owned skill registry over project/user roots, including update and uninstall lifecycle operations.
...
- M7.15 partial: runtime MCP exposes `list_extensions` and `request_extension` control-plane tools for Chariox-managed MCPs and skills. V1 auto-grants valid requests to the current agent and reports when the grant becomes effective. Skill requests now return the full `SKILL.md` body by default, so requested skills can be used in the same turn. Remote worker agents now forward capability discovery/request calls to the home kernel; skill requests return a home-packaged skill directory and materialize it on the worker under `.chariox/remote/skills/<home-kernel-id>/<skill>/<version>/`.
- M7.18: remote skill packaging/materialization is landed for grant-time sync and same-turn `request_extension` use, preserving `SKILL.md`, assets, scripts, and references while skipping provider/cache/build directories and symlinks. Remote prompt dispatch verifies/synchronizes granted skills before submit and injects worker-local `materialized_root` paths into the prompt context.
- M7.20 partial: remote skill live drills pass for OpenCode with the Zen provider model `opencode/gpt-5.2` low effort and Codex with `gpt-5.2` low effort. Remote MCP v1 conformance checks, live conformance drills, and strict provider-native remote Playwright MCP use drills pass: home sends required MCP definitions/hashes for remote prompts, worker checks its own project/user Chariox MCP registry, missing/mismatched/invalid worker MCPs fail fast instead of being installed remotely, and provider-native MCP config is rendered on the worker for Codex/OpenCode. Remote workflow MCP drill coverage now passes for OpenCode and Codex.
...
- Provider capability sync: `/extension import providers` is landed for shared Codex/OpenCode/Claude MCP and skill discovery, deduplication by kind/name/hash, dry-run reporting, and Chariox registry import/update.

Still open in M7:

- Regular non-interactive `chariox mcp ...` / `chariox skill ...` CLI command surfaces, if we keep them separate from slash commands.
- Regular non-interactive agent grant inspection commands, for example `chariox agent mcps` / `chariox agent skills`.
...
- MCP provider activation. V1 uses Chariox-managed provider conversation relaunch/reload rather than relying on provider-global hot reload APIs, preserving the provider-native conversation/thread and re-rendering agent-scoped MCP config.

## Goal

Users should install and manage MCPs and skills through Chariox instead of repeating the same setup in every provider. Agents should receive only the MCPs and skills that were granted to them.

Chariox is an OS/orchestrator, not a provider harness. Third-party MCPs are exposed through each provider's native MCP support. Chariox's runtime MCP remains reserved for Chariox-owned runtime features such as workspace live sync, workflow tools, and capability discovery/request control-plane operations.

## Storage Roots

Chariox-managed MCPs and skills must live outside provider-scanned locations to avoid duplicate exposure and context pollution.
...
Provider-native MCPs and skills remain provider-owned. Import commands copy or register provider definitions into Chariox-owned roots; they do not mutate provider config.

## M7.1 Chariox MCP Config Model

Status: landed for V1 stdio and streamable HTTP config.

Define a Chariox-owned MCP config model aligned with Codex where practical:
...
## M7.2 Chariox MCP Registry

Status: landed for install, list, show, update, and uninstall.

Implement registry operations over Chariox-owned MCP roots:
...
Registry entries are Chariox-owned copies. Installing an MCP registers it; it does not expose it to every agent.
...
```bash
chariox mcp install browser --command npx --arg @playwright/mcp@latest
chariox mcp list
chariox mcp show browser
chariox mcp update browser ...
chariox mcp uninstall browser
```
...
```bash
chariox codex import mcps
chariox codex import mcp browser
chariox opencode import mcps
chariox opencode import mcp github
chariox claude import mcps
```

Behavior:

- read provider-native MCP config
- convert supported definitions into Chariox MCP config
...
- imported MCPs become Chariox-owned copies
...
- imports local MCPs from OpenCode `command` arrays into Chariox stdio MCPs
- imports remote MCPs into Chariox streamable HTTP MCPs
...
Persist per-agent MCP grants. Each agent has an effective MCP set computed from Chariox's registry plus that agent's grants.

Commands:

```bash
chariox agent grant mcp agent-1 browser
chariox agent revoke mcp agent-1 browser
chariox agent mcps agent-1
```

## M7.6 Provider-Native MCP Rendering

Status: partial. Local Codex and OpenCode launches can receive the target agent's granted Chariox MCPs as provider-native MCP config. The isolation spike validated that production should render provider-facing Chariox MCP proxy entries rather than raw backing MCP definitions, so backing MCP processes can remain warm across provider conversation relaunches. Proxy config generation, authenticated proxy dispatch, stdio backing supervision, provider launch proxy rendering, and production HTTP/HTTPS streamable MCP forwarding are landed.

At agent launch, inject only that agent's granted Chariox MCPs into the provider's native MCP config/session overlay.

The provider sees normal MCP servers. Chariox does not expose third-party MCPs as generic Chariox runtime-MCP tools. Production should supervise/proxy MCP backing runtimes while still rendering provider-native MCP entries to Codex/OpenCode.
...
- A provider-facing proxy entry preserves the Chariox MCP's name, enabled/required flags, timeouts, tool allow/deny lists, and per-tool config, but replaces the transport with `streamable_http` pointing at `/mcp/proxy/<mcp-name>`.
...
- `node apps/cli/scripts/live-mcp-skill-drill.mjs --provider opencode --provider-model opencode=opencode/gpt-5.2 --live-mcp-use --timeout-ms 480000 --keep-artifacts-on-failure` passed in 74.3s with Playwright and echo MCPs.
- `node apps/cli/scripts/live-mcp-skill-drill.mjs --providers opencode,codex --provider-model opencode=opencode/gpt-5.2 --provider-model codex=gpt-5.2 --live-mcp-use --timeout-ms 480000 --keep-artifacts-on-failure` passed in 103.4s.
- `node apps/cli/scripts/live-workflow-runtime-drill.mjs --spawn-daemon --scenario mcp-echo-workflow --providers opencode --provider-model opencode=opencode/gpt-5.2 --poll-limit 180 --poll-interval-ms 1000` passed in ~14s.
...
- The drill validates provider-native echo MCP calls, provider-native Playwright/browser MCP calls, Chariox workspace live sync marker writes, same-turn skill requests, web skill install, and agent-triggered MCP request followed by provider conversation relaunch plus automatic continuation.
...
`SKILL.md` should include frontmatter with at least a name and description. Chariox should preserve additional files relative to the skill directory.

## M7.8 Chariox Skill Registry

Status: landed for install, list, show, update, and uninstall.

Implement registry operations over Chariox-owned skill roots:
...
Chariox-managed skills are not stored in `.agents/skills` by default because providers may auto-scan those paths and expose them outside Chariox's per-agent grants.
...
```bash
chariox skill install ./path/to/skill
chariox skill list
chariox skill show browser-qa
chariox skill update browser-qa ...
chariox skill uninstall browser-qa
```
...
```bash
chariox codex import skills
chariox codex import skill browser-qa
chariox opencode import skills
chariox opencode import skill browser-qa
chariox claude import skills
```

Behavior:

- read provider-native skill locations/config
- copy or register supported skills into Chariox-owned skill roots
...
- imported skills become Chariox-owned copies
...
- copies the full skill directory into Chariox-owned skill roots
...
Persist per-agent skill grants. Each agent has an effective skill set computed from Chariox's registry plus that agent's grants.

Commands:

```bash
chariox agent grant skill agent-1 browser-qa
chariox agent revoke skill agent-1 browser-qa
chariox agent skills agent-1
```
...
Inject a short Codex-style available-skills section into every relevant agent prompt/run, listing only that agent's granted Chariox skills.
...
If a Chariox skill declares MCP dependencies, validate that the same agent has those MCPs installed and granted.
...
`list_extensions` returns Chariox-managed MCPs and skills visible from the current workspace, plus whether each one is already granted to the current agent. `request_extension` accepts `kind` (`mcp` or `skill`) and `name`; for v1, valid requests are auto-granted.
...
- Remote skill requests are authorized against the home agent/session and package the home skill directory for the worker. The worker writes the package atomically under `.chariox/remote/skills/<home-kernel-id>/<skill>/<version>/`, verifies file hashes, and adds `materialized_root`, `version_hash`, and file paths to the tool result.
- Remote MCP requests are authorized against home truth, but the worker must already have the matching MCP definition installed in its own Chariox registry. Home sends the expected definition hash before remote launch/prompt; the worker checks project-local roots before user-global roots, so a project-local worker MCP can override a different global MCP. Missing, mismatched, missing-command, and missing-env cases fail fast with a remediation message. V1 does not remotely install MCPs and does not transfer secrets.

Later this plugs into the permissions model instead of always granting.

Hot reload investigation:

- OpenCode has dynamic server-level MCP add/connect routes and rebuilds MCP tools from currently connected MCP clients when constructing prompt tools. That is close to hot reload, but the OpenCode MCP state is server-global. Using it for Chariox per-agent requests would leak a newly requested MCP to other OpenCode sessions/agents served by that OpenCode process unless Chariox first isolates one OpenCode server/process per agent grant scope.
- Codex app-server documents `config/mcpServer/reload`, which reloads MCP server config from disk and queues a refresh for loaded threads on each thread's next active turn. Chariox currently injects granted MCPs through provider launch config, not by mutating Codex's user config on disk. Using this reload safely would require a Chariox-owned, agent-scoped Codex config overlay or a thread-scoped reload API.
- V1 decision: MCP activation uses Chariox-controlled provider relaunch/resume, not provider-global hot reload. Skill hot reload is safe because Chariox can return inert markdown instructions directly through its runtime MCP. OpenCode server-global add/connect must not be used for grant isolation except as a diagnostics path, because it can expose an MCP to other sessions served by the same OpenCode process.
...
- require matching Chariox MCP registry entries on remote machines; v1 does not install them remotely
...
- worker resolves project-local `.chariox/mcps` before user-global `~/.chariox/mcps`
...
- Chariox does not remotely install MCPs and does not transfer MCP secrets in v1
...
- make Chariox-owned skills available to remote worker kernels through home-kernel packaging
...
- inject granted skill summaries/full bodies into the remote agent prompt from Chariox-controlled skill copies
...
- packages whole skill directories from the home registry when a remote worker agent requests a skill through `request_extension`
...
- keeps materialized copies under `.chariox/remote/skills/...`, not provider-scanned skill roots
...
- Registry lifecycle: install one real stdio MCP, install one HTTP MCP when a local server fixture is available, install one public GitHub skill into an isolated `.chariox/skills` drill workspace, list/show/update/uninstall, and verify files are cleaned on success.
...
- Skill same-turn request: create an agent without a skill grant, prompt it to call `list_extensions`, request the public skill, and follow the returned `SKILL.md` body in the same response without waiting for another prompt.
...
- Real web-skill install sources: use public Agent Skills repositories such as <https://github.com/vercel-labs/agent-skills> or curated registries such as <https://github.com/gotalab/skillport> as install candidates, copying into the isolated Chariox skill root rather than provider-scanned `.agents/skills`.
- Provider import: seed Codex/OpenCode native MCP and skill locations with a fixture, import into Chariox-owned roots, then verify grants/rendering come from Chariox-owned copies and not provider-scanned duplicates.
...
```bash
node apps/cli/scripts/live-remote-skill-drill.mjs --provider opencode --model opencode/gpt-5.2 --effort low
node apps/cli/scripts/live-remote-skill-drill.mjs --provider codex --model gpt-5.2 --effort low
```

Observed on 2026-04-18: both drills passed. The drill creates isolated relay/home/worker daemons, installs a Chariox-owned skill with an asset, verifies local-only grants do not materialize on the worker, moves a pre-granted local agent to remote and verifies grant synchronization, verifies remote-first grant-time worker materialization, deletes the worker copy and verifies prompt-time repair, verifies same-turn remote `request_extension` skill materialization, submits live remote prompts for each live scenario, and verifies the provider wrote `outputs/remote-skill-provider.txt` with the asset token and `REMOTE_SKILL_DRILL_OK`.

Remote MCP conformance drill:

```bash
node apps/cli/scripts/live-remote-mcp-drill.mjs --provider opencode --model opencode/gpt-5.2 --effort low
node apps/cli/scripts/live-remote-mcp-drill.mjs --provider codex --model gpt-5.2 --effort low
```

Observed on 2026-04-18: both conformance drills passed. The drill creates isolated relay/home/worker daemons with separate `HOME` roots, installs divergent Chariox MCP definitions into home and worker registries, verifies worker-missing MCP failure, worker global definition mismatch failure, project-local worker override success, missing worker stdio command failure, and missing worker env var failure.

Strict provider-native remote MCP use drill:

```bash
node apps/cli/scripts/live-remote-mcp-drill.mjs --provider opencode --model opencode/gpt-5.2 --effort low --timeout-ms 300000 --live-mcp-use
node apps/cli/scripts/live-remote-mcp-drill.mjs --provider codex --model gpt-5.2 --effort low --timeout-ms 300000 --live-mcp-use
```

Observed on 2026-04-18 after production HTTPS/chunked MCP proxy support landed: both strict drills passed. OpenCode completed a provider-native `playwright_browser_snapshot` call and Codex completed a provider-native `browser_tabs` call on the worker. Both then wrote `outputs/remote-playwright-mcp.txt` with exactly `M7_REMOTE_PLAYWRIGHT_MCP_OK` through Chariox workspace live sync. The drill preserves real provider auth/config/cache while keeping isolated Chariox home/worker registries by passing through `CODEX_HOME`, `OPENCODE_CONFIG_DIR`, and `XDG_*` provider paths.

Remote workflow MCP drill:

```bash
node apps/cli/scripts/live-remote-workflow-runtime-drill.mjs --scenario mcp-echo-workflow --provider opencode --model gpt-5.2 --provider-model opencode=opencode/gpt-5.2 --poll-limit 240 --poll-interval-ms 1000
...
Observed on 2026-04-19: negative remote workflow drills now surface provider diagnostics in the workflow failure event instead of falling back to generic missing-output state. Codex command: `node apps/cli/scripts/live-remote-workflow-runtime-drill.mjs --scenario mcp-echo-workflow --provider codex --model gpt-5.2 --poll-limit 120 --poll-interval-ms 1000`. Result: workflow status `Failed`, node status `Failed`, failure event `provider_failure`, and message includes the Codex HTTP 400 error: `The 'gpt-5.2-codex' model is not supported when using Codex with a ChatGPT account.` OpenCode command: `node apps/cli/scripts/live-remote-workflow-runtime-drill.mjs --scenario mcp-echo-workflow --provider opencode --model zen/gpt-5.2 --poll-limit 80 --poll-interval-ms 1000`. Result: workflow status `Failed`, node status `Failed`, failure event `provider_failure`, and message includes `Model not found: zen/gpt-5.2` / `ProviderModelNotFoundError`. Both drill processes exit nonzero because the scenario normally expects success; the failure state and provider diagnostic are the expected negative-drill evidence.

## V2 Remote Capability Work

Remote MCP/skill support is v1-complete with explicit limits: home owns registry truth, skills are materialized to workers as needed, and MCPs must already exist on the worker with a matching definition/version. V2 should add the remote-management layer that v1 intentionally avoided:

- Remote MCP install from home: let Chariox install a missing MCP on a worker from the home-owned registry, using an explicit user-approved operation and worker-local execution context.
- Remote MCP version repair: when a worker has a missing or mismatched MCP definition, offer a Chariox-managed repair path that installs or updates the worker/project-local definition to match home instead of only failing fast.
- Nightly remote drill suite: promote the current local/remote MCP and skill drills, including expected-failure diagnostics for Codex/OpenCode provider errors, into a repeatable nightly suite with cleanup guarantees for daemons, sessions, ports, generated files, and provider temp state.
