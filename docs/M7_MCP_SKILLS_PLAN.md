...
- Provider capability sync: `/extension import providers` is landed for shared Codex/OpenCode/Claude MCP and skill discovery, deduplication by kind/name/hash, dry-run reporting, and Arroba registry import/update.
...
`list_extensions` returns Arroba-managed MCPs and skills visible from the current workspace, plus whether each one is already granted to the current agent. `request_extension` accepts `kind` (`mcp` or `skill`) and `name`; for v1, valid requests are auto-granted.
...
- Remote MCP install from home: let Arroba install a missing MCP on a worker from the home-owned registry, using an explicit user-approved operation and worker-local execution context.
- Remote MCP version repair: when a worker has a missing or mismatched MCP definition, offer an Arroba-managed repair path that installs or updates the worker/project-local definition to match home instead of only failing fast.
