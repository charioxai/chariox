...
Observed on 2026-04-19: the negative remote Codex drill using bare `--model gpt-5.2` now fails visibly instead of staying `Running`. Command: `node apps/cli/scripts/live-remote-workflow-runtime-drill.mjs --scenario mcp-echo-workflow --provider codex --model gpt-5.2 --poll-limit 120 --poll-interval-ms 1000`. Result: workflow status `Failed`, node status `Failed`, failure event `missing_structured_output` with message `provider completed workflow turn without a validated workflow output`. The drill process exits nonzero because the scenario normally expects success; the state transition is the expected negative-drill evidence.
