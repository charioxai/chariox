...
Status of the open Apps Phase 1 PR stack. Nothing below is merged yet: the OSS stack is #433 → #434 → #435 → #436 → #438 → #439 → #441 → #442 → #443 → #444 → #445 → #446 → #447 → #455 → #456 → #457 → #458 → #459 → #460 → #461 → #462 → #463 → #464 → #470 → #471 → #473. Side fixes #467, #468, #472 Side PRs are #430, #431, #437, #440, #448, #452. Web work is in chariox-cloud #218 → #220 → #221 → #222.
...
| P1.04 | Partial | foundation, #444 (effects), #462 (file grants), #463 (export) | Live state, files and HTTP broker. User-selected file grants: live prompt in the web terminal and grant; `files.import` tested through a real worker. Live `files.export` from Documents, saved through the web prompt | `files.snapshot` |
...
| P1.07 | Implemented | #436, #470, Cloud #227 | Live drill (`live-app-view-drill.mjs`): view calls, `UNKNOWN_TOOL`, 20 calls overlapping Room commands with no busy refusals. Live accessibility outline of the Todo view (protocol 357): web "Read as text" and TUI `/room read` list the form, the open count and every Todo with its Done checkbox and Delete button, in document order | Live screen-reader pass (VoiceOver) over the web outline |
| P1.08 | Implemented | #439, #442, #446 | `/app` commands, logs, dev loop. Live TUI drill (real TUI in a pseudo-terminal): attach, `/room status`, `/room read`, `/room view` opening the web viewer on the same Room, and a TUI prompt that makes the focus agent add a Todo through the App's tools | — |
...
| P1.10 | Partial | #445, #455, #458, #460, #461, #464, #465, #466, #469, #471 | Live local updates (Todo 1.0.1 to 1.1.0, then 1.2.0); live migration on the enrolled runtime (Todo 1.2.0 with `migrations/001.js`, whose chain packing derived from the numbered file (#465): every Todo gained its new field, and writes at schema 1 work); kernel tests for migration commit and restore; wakes and inbox deliveries (#460) wait through an update; a kernel stop right after an update quiesced finishes the update on restart, or restores the old generation and its data when the new one fails (#461); open views reconnect to the new generation after an update or a kernel restart, and Documents keeps unsaved edits across that reload (#464; live on a slice runtime cross-built from this stack: a Todo view left open across a kernel restart took its next Add without being reopened; a worker left failed by an earlier start error stays failed until `/app start`, by design); an update that changes a routed event's schema marks its automation broken and logs the accepted events it will not deliver (#466); a failed update restores the committed generation's data snapshot, also after several failed updates in a row: macOS file-image snapshot (#469, live with Todo and Documents), Linux root helper (#471, hosted fixture in a disposable Ubuntu 24.04 VM) | `files.snapshot` (App-initiated); recovery drills at the remaining checkpoints (a stop mid-migration with partial writes, a stop between commit and activation) |
...
| P1.13 | Implemented | #441, #462, #463, Cloud #224, #225 | Live create and edit of documents, versions. Import: Documents asks for files, the owner chooses them in the web terminal's trusted prompt, and the document appears in Documents (live on the re-enrolled runtime). Export: a document is offered and saved from the web prompt | — |
...
| P1.18 | Phase 2 | #430 | — | — |
| P1.19 | Partial (App-created assets are Phase 2) | #447, #456, #457, Cloud #220 | Live single-App acceptance (Todo, web terminal): foreground binding; a Claude agent creates a Todo through the one prompt area; a second agent resumes it from App-owned state; closing and reopening the view keeps App state and kernel history, and the App DOM never contains the conversation | Live from the TUI too: the focus agent, prompted in the TUI, adds a Todo through the App's tools; `/room view` opens the Room's viewer (#467, #472, #473 fixed what the TUI drill found) |
