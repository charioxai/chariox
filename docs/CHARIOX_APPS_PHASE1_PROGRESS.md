...
| P1.10 | Partial | #445, #455, #458, #461 | Live local updates (Todo 1.0.1 to 1.1.0); live migration on the enrolled runtime (Todo 1.2.0 with `migrations/001.js`: every Todo gained its new field, and writes at schema 1 work); kernel tests for migration commit and restore; wakes and inbox deliveries wait through an update; a kernel stop right after an update quiesced finishes the update on restart, or restores the old generation and its data when the new one fails; open views reconnect to the new generation after an update or a kernel restart, and Documents keeps unsaved edits across that reload (#464; live on a slice runtime cross-built from this stack: a Todo view left open across a kernel restart took its next Add without being reopened. A worker left failed by an earlier start error stays failed until `/app start`, by design) | `files.snapshot` (App-initiated). Packing derives migrations from numbered files (#465); an update that changes a routed event's schema marks its automation broken (#466); on macOS a failed update keeps the active generation startable and restores the data image from its file-image snapshot (#469, live with Todo and Documents); on Linux the root storage helper keeps the same data snapshot across a staged update and restores it when the update fails (#471, hosted helper fixture in a disposable Ubuntu 24.04 VM) |
