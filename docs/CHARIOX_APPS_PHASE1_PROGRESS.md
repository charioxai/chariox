...
| P1.06 | Partial | #433 (wakes), outbox foundation, #460 (inbox) | Outbox forwarding to workflows; kernel wakes. Live inbox: route, dedupe and conflict, schema refusal, `events.deliver` creates a Todo, and delivery waits for a user-stopped worker | Event generator subscriptions and AEDS acceptance for inbox routes (with P1.11) |
