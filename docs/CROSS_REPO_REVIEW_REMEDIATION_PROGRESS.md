...
- Phase 2 remaining:
  - **C2 sync feed** — enforcement primitive is complete: `RelayRevocationRegistry` revokes by `jti`, account, and client/machine subject (matching the cloud's `Revocation` model: accountId + subjectKind + subject), wired via `ScopedTokenVerifier::with_revocations`. Remaining: a cloud endpoint listing active revocations and a relay periodic pull that calls `revoke_token_id`/`revoke_account`/`revoke_subject`/`prune`. Needs a relay HTTP client + cloud endpoint; verify the end-to-end flow live.
