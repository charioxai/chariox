...
Artifact-index gates can enforce the same provenance with `--require-artifact-generated-evidence-kind validation-suite-run --require-artifact-generated-evidence-kind matrix-report`; artifact-index aggregates preserve `generatedValidationSuiteFailureRoots`, and validation-gate aggregate coverage exposes the same roots as `artifactGeneratedValidationSuiteFailureRoots`, so downstream staging jobs can route preserved generated suite failures without opening every source index. Output artifact metadata also records generated evidence kinds, generated roots, and generated validation-suite failure roots, so downstream staging jobs can distinguish gate-generated evidence from discovered or explicitly supplied evidence. Cloud staging smoke reports add artifact hints for the generated validation-suite and matrix roots when `--generate-distributed-runtime-gate-evidence` is enabled.
