...
- Mark interrupted/recovering work correctly. **Superseded:** the original M8 implementation cleared active prompts and stopped workflows after a crash. Crash bootstrap now preserves prompt, workflow, node, and scheduler authority for transcript-aware restart reconciliation; only explicit graceful shutdown uses interruption cleanup. See `DURABLE_RUNTIME_RESILIENCE_PROGRESS.md` Milestones 5 and 6.
- Validate local restart drills. **Landed, then strengthened:** the original drill proved durable session/config/history restoration with interrupted work cleanup. The current `SIGKILL` drills additionally prove the same active prompt, provider identity, running workflow/node/scheduler state, grants, transcript, and recall survive process restart without an interruption failure. The CLI reconnect drill remains the surface-level disconnect/reattach check.
