...
- Rebuild projections from durable state. **Started:** bootstrap now replays `session.created`, `session.updated`, `agent.created`, `session.ended`, `session.deleted`, agent MCP/skill grant mutation events, and provider runtime-profile update events, restores session/agent stores, clears live agents for ended sessions, removes deleted sessions, and refreshes session projections.
