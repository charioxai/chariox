...
- [ ] **M4.5-001** Implement the kernel runtime refactor
  ...
  - Progress: Kernel command/event envelopes, event replay gaps, projection metadata, command routing, bounded interactive routing, CLI replay-gap handling, command-id retry/fanout safety, inbound WebSocket request bounds, provider-run actor isolation, runtime cleanup tombstones, reserved-listener websocket tests, `KernelSessionService` lifecycle/focus/resize/end/delete ownership, `KernelAgentService` prompt submit/cancel/complete/queue lifecycle ownership, per-agent prompt command mailboxes including completion/cancellation routing with agent-runtime active-owner projection resolution, per-session attach/focus/resize/end/delete command mailboxes with close cleanup and projection-backed delete/detach lane lookup, projected focus fallback for untargeted prompt routing, projection-first reads for session/list/resolve/history/provider-run/process/catalog plus agent/workflow inspection, list-hydrated session-state projection reads, direct agent-runtime active/queued prompt projection reads including queue-advance and provider-settlement inspection, agent-mailbox prompt submit/cancel/complete projection publication from the shared projection store without a duplicate private prompt-state shadow, compatibility prompt mutation concentrated in a dedicated flattened `PromptRuntimeState` session prompt-runtime module, removal of the unused direct complete-and-auto-advance mutation API, direct compatibility complete/cancel owner resolution aligned with agent-runtime active-owner rules, prompt lifecycle projection publication for complete/cancel, trimmed router-side snapshots for non-state terminal control commands, provider actor enqueue error propagation, canonical agent-runtime prompt-count projections in daemon health with legacy session-counter mirroring, exposed daemon health queue snapshots, explicit file-writing capability worktree claims, provider prompt lifecycle worktree claims, and workflow node dispatch blocking/retry on workspace claims are now landed.
  - Next: Move `PromptRuntimeState` behind `AgentRuntime`, then move session state ownership into mailbox runtimes, make reads projection-first, and remove remaining hot-path `Arc<Mutex<DaemonApp>>` dependencies. Near-term A+ sequence: finish prompt ownership, session ownership, projection correctness, workflow runtime hardening, provider/terminal hardening, hot app-lock removal, docs/invariant lock, then final I/O coordination. Keep the current workspace claims as a coarse safety/scheduler layer for now; defer file-level claims, port claims, harness sandboxing, and transactional patch/rebase coordination until the final I/O-coordination slice.
