...
- [ ] **M4.5-001** Implement the kernel runtime refactor
  ...
  - Next: Final M4.5 docs/invariant alignment. The seven ownership points are closed: direct-cutover baseline, session ownership, prompt ownership, provider process/output ownership, workflow/runtime-tool ownership, transport/relay ownership, and runtime fallback deletion now use owned runtime paths. The M4.5 dead-code purge removed unused app-backed helper surfaces and obsolete runtime-tool compatibility dispatch. Keep the current workspace claims as a coarse safety/scheduler layer while deferring file-level claims, port claims, harness sandboxing, coordinator-owned patch application, and transactional mutation/rebase coordination to the final I/O slice.

- [ ] **M4.6-001** Implement coordinated agent I/O
  - Note: Add kernel-owned artifact read/write coordination described in `docs/M4_6_COORDINATED_AGENT_IO_PLAN.md`. Arroba remains unopinionated about branches/worktrees, but managed provider sessions must not be able to bypass coordinated writes; first supported providers must prove direct workspace writes are blocked and edits route through managed MCP/runtime tools.
