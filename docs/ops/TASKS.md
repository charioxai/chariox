...
- [x] **M4.6-001** Implement coordinated agent I/O
  - Note: Landed kernel-owned managed I/O for Arroba-launched provider sessions. Supported providers cannot write coordinated workspace files except through Arroba MCP/runtime tools; Codex and OpenCode enforcement is covered by tests and live drills. V1 supports fine-grained text coordination, external-change detection/rebase/rejection semantics, local and remote home-kernel coordination, and opaque whole-file locking for non-text artifacts.
  - Commit: `5d1cb0d`
