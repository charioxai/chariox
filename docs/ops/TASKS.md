...
- [ ] **M4.5-001** Implement the kernel runtime refactor
  ...
  - Progress: Kernel command/event envelopes, event replay gaps, projection metadata, command routing, bounded interactive routing, CLI replay-gap handling, command-id retry/fanout safety, inbound WebSocket request bounds, provider-run actor isolation, runtime cleanup tombstones, reserved-listener websocket tests, `KernelSessionService` lifecycle/focus/resize/end/delete ownership, `KernelAgentService` prompt submit/cancel/complete/queue lifecycle ownership, per-agent prompt command mailboxes, per-session attach/focus/resize/end/delete command mailboxes with close cleanup, exposed daemon health queue snapshots, explicit file-writing capability worktree claims, and provider prompt lifecycle worktree claims are now landed.
  - Next: Move session/agent state ownership into the mailbox runtimes, make reads projection-first, extend workspace coordination to workflow node dispatch/file-level/port claims, and remove remaining hot-path `Arc<Mutex<DaemonApp>>` dependencies.
