...
- [ ] **M4.5-001** Implement the kernel runtime refactor
  ...
  - Progress: Kernel command/event envelopes, event replay gaps, projection metadata, command routing, bounded interactive routing, CLI replay-gap handling, command-id retry/fanout safety, inbound WebSocket request bounds, provider-run actor isolation, runtime cleanup tombstones, reserved-listener websocket tests, `KernelSessionService` lifecycle/focus/resize ownership, and `KernelAgentService` prompt submit/cancel/complete/queue lifecycle ownership are now landed.
  - Next: Convert `KernelSessionService` and `KernelAgentService` into true session/agent actor ownership, make reads projection-first, add daemon health/queue projections, and remove remaining hot-path `Arc<Mutex<DaemonApp>>` dependencies.
