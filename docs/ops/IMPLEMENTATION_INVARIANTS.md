...
- Mutable prompt lifecycle changes enter through `KernelAgentService` or the `AgentRuntime` mailbox path. Session prompt fields are compatibility mirrors until prompt state is fully actor-owned.
