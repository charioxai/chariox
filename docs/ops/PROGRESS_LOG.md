...
- Remaining validation is intentionally limited to external blockers: OpenCode Workspace Live Sync drills still fail before sync behavior with `Token refresh failed: 401`, and Scalingo/staging hosted plus hosted second-user drills are deferred until the platform issue is cleared.
