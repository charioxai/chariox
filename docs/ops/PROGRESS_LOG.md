...
- `config workspace-live-sync` normalizes `on|off` to `managed|unrestricted` and writes the same user-config key as the TUI command, while reporting that shell changes apply on the next provider launch.
