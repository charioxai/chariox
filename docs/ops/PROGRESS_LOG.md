...
- Added the first agent runtime projection store. Session projection refreshes now materialize per-agent active and queued prompt counts, and `GetDaemonHealth` exposes those counts as an agent-runtime projection view.
