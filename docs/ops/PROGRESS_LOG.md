...
- Rechecked `pnpm --filter @arroba/cli run workspace-live-sync:opencode-managed-drill` on current `main`; OpenCode still fails before Workspace Live Sync behavior with provider auth `Token refresh failed: 401`. No failed-drill relay/kernel/provider processes or transient drill roots were left behind.
