...
- Routed runtime MCP HTTP through `CommandRouter`. The MCP server now binds from the router-owned config projection and dispatches authenticated workflow runtime tools through the router instead of locking `DaemonApp` directly from the transport layer.
