...
- Moved Slack, Discord, Telegram, WhatsApp, and Signal connectors into M9 V1 scope and clarified the two-layer auth model: connector ingress verification proves provider/transport authenticity, while Arroba identity authorization handles registered users, teams, paired senders, API token owners, or anonymous callers.
