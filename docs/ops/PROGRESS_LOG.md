...
- Routed relay-client daemon requests through `CommandRouter` as `KernelCommandSource::RelayClient`. Proxied relay clients now share the same actor admission, projection refresh, and overload behavior as local IPC/kernel transport requests, and regression coverage verifies relay list requests warm the shared session projection.
