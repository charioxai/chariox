...
- Rechecked `pnpm --filter @arroba/cli run workspace-live-sync:opencode-managed-drill` on current `main` HEAD `6cf2ee80`; OpenCode still fails before Workspace Live Sync behavior with provider auth `Token refresh failed: 401`. No failed-drill relay/kernel/provider processes or transient drill roots were left behind.
...
- Full OpenCode managed parity is still open. A full managed run with `opencode/gpt-5.2` progressed through write/edit/move/opaque/delete/direct-write/collision, then Arroba history stopped at a pending `arroba_read_artifact` update during the external non-overlap phase. The OpenCode DB showed the same tool completed and emitted `OPENCODE_EXTERNAL_NONOVERLAP_READ_DONE`, so the remaining issue is OpenCode event/snapshot projection into Arroba after the two-agent collision phase.
- OpenCode tracked parity is also still open. A tracked positive-only run with `opencode/gpt-5.2` reached the source-origin prompt, but OpenCode used its native `read` tool and then replied that it could not assist instead of performing the requested direct filesystem writes. Updated OpenCode Workspace Live Sync aliases to use `opencode/gpt-5.2` so future runs exercise the working provider path instead of the stale OpenAI OAuth credential.
...
- Remaining validation is intentionally limited to external blockers: OpenCode Workspace Live Sync drills still fail before sync behavior with `Token refresh failed: 401` when rechecked through `pnpm --filter @arroba/cli run workspace-live-sync:opencode-managed-drill`, and Scalingo/staging hosted plus hosted second-user drills are deferred until the platform issue is cleared.
