...
- Local OpenCode native TUI: `pnpm --filter @arroba/cli run native-tui:opencode-drill`. The drill launches two `arroba opencode` native TUIs into one Arroba session plus one observer Arroba CLI, validates native-CLI and Arroba-CLI prompt submission in both directions, checks per-agent output separation, and observes native-TUI agent badges moving from idle to working and back to idle.
...
- Local OpenCode native TUI: **pass**. On 2026-05-13, `pnpm --filter @arroba/cli run native-tui:opencode-drill` passed with two provider-native OpenCode TUIs and one observer Arroba CLI in the same Arroba session. It validated provider-TUI prompts and Arroba-CLI prompts for both native agents, no cross-agent marker contamination, and badge transitions `IDLE -> THINKING/working -> IDLE`.
