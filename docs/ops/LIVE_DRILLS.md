...
- Remote MCP management: pending drill. V1 behavior is conformance-only: the worker must already have the matching MCP definition installed in its project/user Arroba registry, and missing/mismatched/missing-command/missing-env cases fail fast before remote provider launch/prompt.
