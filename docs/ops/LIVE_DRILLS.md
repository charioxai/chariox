...
- Local shell/scriptability: **pass**. On 2026-04-19, `pnpm --filter @arroba/cli run shell:drill` passed against an isolated local kernel. The drill created a standalone shell-attached session, spawned dev-stub agents, exercised config/MCP/skill/workflow graph/workflow config/watchdog/queue/provider-process commands, and verified script seed variables, nested `source <file>` loading, and `--continue-on-error` handling for TUI-only, no-active-prompt, and missing-relay failures.
- Embedded workflow shell: **graph/script pass, runtime provider-stub-limited**. On 2026-04-19, a manual TUI drill launched an isolated kernel and CLI, opened `/workflow`, created agents and a workflow through `@` shell commands, and verified the workflow pane updated to `nodes=2`, `edges=1`, `endpoints=1`. A second TUI drill loaded the same graph from `@ source source-flow.arroba`; the pane switched to the loaded `source-flow` workflow and showed the same node/edge/endpoint counts. Running the graph with `dev-stub` failed with `missing_structured_output` because the generic stub does not emit workflow JSON; end-to-end provider execution remains covered by the local workflow runtime drills.
