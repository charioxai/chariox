...
- Cloud multi-user bridge: **partially covered**. On 2026-04-21, command/unit coverage passed for cloud invite create/accept and shell command wiring, the cloud repo Prisma smoke drill passed the two-cloud-user invite/member/collaborator path, and `pnpm --filter @arroba/cli run cloud-relay:drill` still passes the hosted cloud relay login/pair/connect/client-token path. The remaining live gap is the full hosted-relay two-user workflow drill through cloud-issued credentials.
