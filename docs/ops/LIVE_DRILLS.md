...
- Local runtime MCP reattach: **pending after runtime MCP provider-attachment fix**. Run `node apps/cli/scripts/live-runtime-mcp-reattach-drill.mjs --providers opencode,codex --provider-model opencode=openai/gpt-5.2 --timeout-ms 360000 --poll-ms 1000`. This drill warms provider catalog endpoints before managed-I/O launch, then detaches and reattaches the CLI, and fails unless each provider uses Arroba runtime MCP `list_capabilities` plus `read_artifact` both before and after reattach.
...
- Remote MCP management: **strict pass**. On 2026-04-18, `node apps/cli/scripts/live-remote-mcp-drill.mjs --provider opencode --model openai/gpt-5.2 --effort low --timeout-ms 300000 --live-mcp-use` passed in ~146s and `node apps/cli/scripts/live-remote-mcp-drill.mjs --provider codex --model gpt-5.2 --effort low --timeout-ms 300000 --live-mcp-use` passed in ~89s. Both created isolated relay/home/worker daemons with separate `HOME` roots, verified worker-missing MCP fails fast, verified worker global definition mismatch fails fast, verified worker project-local matching MCP overrides a mismatched global MCP, verified missing worker stdio command fails fast, verified missing worker env var fails fast, verified a provider-native remote Playwright/browser MCP tool call on the worker, and wrote `outputs/remote-playwright-mcp.txt` with `M7_REMOTE_PLAYWRIGHT_MCP_OK` through Arroba managed I/O. OpenCode used `playwright_browser_snapshot`; Codex used `browser_snapshot`.
