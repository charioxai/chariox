...
- Local MCP/skill management: **partial pass**. `node apps/cli/scripts/live-mcp-skill-drill.mjs --provider codex --require-web-skill --timeout-ms 240000 --poll-ms 1000` passed on 2026-04-17: installed real Playwright MCP, installed public `vercel-labs/agent-skills` skill `deploy-to-vercel`, installed deterministic drill skill, granted capabilities to a Codex agent, verified pre-granted skill prompt use, and verified same-turn `request_capability` returns a skill body that the agent follows immediately. `node apps/cli/scripts/live-mcp-skill-drill.mjs --providers opencode,codex --skip-live-provider --require-web-skill --include-github-mcp` also passed registry/web-skill install coverage; GitHub MCP was skipped because no GitHub token env var was set. The stricter `--live-mcp-use` path is still blocked: Codex reports Playwright MCP unavailable from the active provider run, matching the known next-provider-launch/relaunch gap.
