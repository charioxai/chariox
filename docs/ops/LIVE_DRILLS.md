...
- Cloud multi-user bridge: **covered for hosted session entry**. On 2026-04-21, command/unit coverage passed for cloud invite create/accept and shell command wiring, the cloud repo Prisma smoke drill passed two-cloud-user invite/member/collaborator plus session-scoped token issuance/denial, and `pnpm --filter @arroba/cli run cloud-relay:drill` passed a two-cloud-user hosted relay join using a session-scoped cloud-issued relay token. Remaining hardening: rerun the M6.5 workflow/redaction assertions through cloud-issued session-scoped relay tokens.
