...
- Remote MCP management: `node apps/cli/scripts/live-remote-mcp-drill.mjs --provider opencode --model openai/gpt-5.2 --effort low` and `node apps/cli/scripts/live-remote-mcp-drill.mjs --provider codex --model gpt-5.2 --effort low`. V1 behavior is conformance-only: the worker must already have the matching MCP definition installed in its project/user Arroba registry, and missing/mismatched/missing-command/missing-env cases fail fast before remote provider launch/prompt.
...
- Remote MCP management: **pass**. On 2026-04-18, `node apps/cli/scripts/live-remote-mcp-drill.mjs --provider opencode --model openai/gpt-5.2 --effort low --timeout-ms 180000` and `node apps/cli/scripts/live-remote-mcp-drill.mjs --provider codex --model gpt-5.2 --effort low --timeout-ms 180000` passed. Both created isolated relay/home/worker daemons with separate `HOME` roots, verified worker-missing MCP fails fast, verified worker global definition mismatch fails fast, verified worker project-local matching MCP overrides a mismatched global MCP, verified missing worker stdio command fails fast, and verified missing worker env var fails fast.
