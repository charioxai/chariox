...
use support::runtime_integration::{
    ...
    create_opencode_fixture_script, opencode_env_guard, output_timeout_ms, render_terminal_output,
    wait_for_mock_opencode_event_subscription, wait_for_provider_runtime_state, MockOpenCodeServer,
};
...
fn focused_agent_prompts_route_to_distinct_opencode_runs_and_history() {
    ...
    let first_submission = arroba_kernel::transport::TransportService::schedule_direct_prompt(
        &mut app,
        session.id(),
        attachment.id(),
    ...
    let second_submission = arroba_kernel::transport::TransportService::schedule_direct_prompt(
        &mut app,
        session.id(),
        attachment.id(),
        "review agent prompt\n",
...
fn focusing_another_agent_during_an_opencode_prompt_keeps_the_working_run_active() {
    ...
    app.focus_agent(session.id(), default_agent.id())
        .expect("default agent should focus");
    let started = arroba_kernel::transport::TransportService::schedule_direct_prompt(
        &mut app,
        session.id(),
        attachment.id(),
