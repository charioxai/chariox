...
#[test]
fn ordinary_lease_caller_binding_survives_restart_and_rejects_spoofs() {
    run_async_with_large_test_stack(
        "ordinary-lease-caller-binding",
        ordinary_lease_caller_binding_survives_restart_and_rejects_spoofs_async,
    );
}

async fn ordinary_lease_caller_binding_survives_restart_and_rejects_spoofs_async() {
    let _relay_test_guard = relay_client_test_guard().await;
    let _test_home = RelayTestHome::new();
    let test_root = std::env::temp_dir().join(format!(
        "chariox-ordinary-lease-caller-{}-{}",
        std::process::id(),
    ...
    assert_unauthorized(
        send_peer_request_via_relay(
            ...
            RelayPeerRequest::CreateExecutionLease {
                home_kernel_id: "forged-home-kernel-id".to_string(),
    ...
    assert_unauthorized(
        send_peer_request_via_relay(
            ...
            RelayPeerRequest::SteerLeasedPrompt {
                leased_agent_id: leased_agent.id.clone(),
                steer_id: "other-home-steer-attempt".to_string(),
                target_home_prompt_id: target_home_prompt_id.to_string(),
                prompt: "untrusted steer must not be accepted".to_string(),
                hidden_system_context: String::new(),
                attachments: Vec::new(),
                required_skills: None,
            },
    ...
    // The relay daemon ID alone is not the lease authority: replacing the
    ...
    assert!(
        registry
            ...
            .daemon(&home_config.daemon_id)
            .is_none(),
        "original home registration must be removed before same-ID replacement"
    );
    let mut same_id_other_key_config = isolate_test_config(
        DaemonConfig::for_tests(),
        &test_root,
        "same-id-other-key-home",
    );
    same_id_other_key_config.daemon_id = home_config.daemon_id.clone();
    same_id_other_key_config.host_machine_id = home_config.host_machine_id.clone();
    same_id_other_key_config.relay_url = home_config.relay_url.clone();
    ...
    let app_same_id_other_key = Arc::new(Mutex::new(
        DaemonApp::bootstrap(same_id_other_key_config.clone())
            .expect("same-ID replacement home should bootstrap"),
    ));
    let state_same_id_other_key = Arc::new(RwLock::new(RelayClientState::default()));
    let (shutdown_same_id_other_key_tx, shutdown_same_id_other_key_rx) = watch::channel(false);
    let connector_same_id_other_key = tokio::spawn(run_daemon_relay_connector(
        Arc::clone(&app_same_id_other_key),
        Arc::clone(&state_same_id_other_key),
        shutdown_same_id_other_key_rx,
    ));
    wait_for_daemon_registration(registry.clone(), &same_id_other_key_config.daemon_id).await;
    assert_eq!(
        registry
            .read()
            .await
            .daemon(&same_id_other_key_config.daemon_id)
            .expect("same-ID replacement must be registered")
            .public_key,
        same_id_other_key_config.relay_public_key,
        "relay must register the replacement's distinct encrypted sender key"
    );
    let same_id_target = ClientTarget {
        daemon_id: Some(worker_config.daemon_id.clone()),
        daemon_alias: None,
    };
    assert_unauthorized(
        send_peer_request_via_relay(
            &app_same_id_other_key,
            &state_same_id_other_key,
            same_id_target.clone(),
            RelayPeerRequest::GetLeasedPromptReceipt {
                leased_agent_id: leased_agent.id.clone(),
                home_prompt_id: steer_id.to_string(),
            },
    ...
    assert_unauthorized(
        send_peer_request_via_relay(
            ...
            RelayPeerRequest::SteerLeasedPrompt {
                leased_agent_id: leased_agent.id.clone(),
                steer_id: "same-id-new-key-steer-attempt".to_string(),
                target_home_prompt_id: target_home_prompt_id.to_string(),
                prompt: "different encryption key must not steer".to_string(),
                hidden_system_context: String::new(),
                attachments: Vec::new(),
                required_skills: None,
            },
    ...
    let _ = shutdown_same_id_other_key_tx.send(true);
    connector_same_id_other_key
        .await
        .expect("same-ID replacement connector should stop");
    for _ in 0..80 {
        if registry
            .read()
            .await
    ...
    assert!(
        registry
            ...
            .daemon(&home_config.daemon_id)
            .is_none(),
        "same-ID replacement registration must be gone before restoring original home"
    );
    let (shutdown_home_restarted_tx, shutdown_home_restarted_rx) = watch::channel(false);
    let connector_home_restarted = tokio::spawn(run_daemon_relay_connector(
        Arc::clone(&app_home),
        Arc::clone(&state_home),
        shutdown_home_restarted_rx,
    ));
    wait_for_daemon_registration(registry.clone(), &home_config.daemon_id).await;
    ...
    let _ = shutdown_home_restarted_tx.send(true);
    let _ = shutdown_restarted_tx.send(true);
    let _ = shutdown_other_home_tx.send(true);
    connector_home_restarted
        .await
        .expect("restored original home connector should stop");
    restarted_connector
        .await
    ...
    drop(app_other_home);
    drop(state_same_id_other_key);
    drop(app_same_id_other_key);
