...
fn local_daemon_protocol_workspace_live_sync_status_shape_is_versioned() {
    assert_eq!(LOCAL_DAEMON_PROTOCOL_VERSION, 55);
    ...
    assert_eq!(
        snapshot.pointer("/1/WorkspaceLiveSyncStatus/status/footer_state"),
        Some(&serde_json::json!("tracked"))
    );
    assert_eq!(
        snapshot.pointer("/1/WorkspaceLiveSyncStatus/status/targets/0/status"),
    ...
    assert_eq!(
        format!("{hash:x}"),
        "a61c339a3e879916dd711120c88241e9db3923a9769afc2dd43feb92dbf6d28b"
    );
}
