...
fn local_daemon_protocol_waiting_room_activity_shape_is_versioned() {
    assert_eq!(LOCAL_DAEMON_PROTOCOL_VERSION, 173);
...
fn local_daemon_protocol_workflow_code_shape_is_versioned() {
    ...
    assert_eq!(
        snapshot.pointer("/7/WorkflowCodeValidated/result/validation/ok"),
        Some(&serde_json::json!(true))
    );
    assert_eq!(
        snapshot.pointer("/7/WorkflowCodeValidated/result/definition/workflow/max_concurrent"),
        Some(&serde_json::json!(32))
    );
    assert_eq!(
        snapshot.pointer("/8/WorkflowCodeApplied/result/apply/node_ids/planner"),
        Some(&serde_json::json!("node-1"))
    );
    assert_eq!(
        snapshot.pointer("/8/WorkflowCodeApplied/session/id"),
        Some(&serde_json::json!("session-1"))
    );
    assert_eq!(
        snapshot.pointer("/9/WorkflowCodeArtifactCreated/artifact/metadata/name"),
        Some(&serde_json::json!("toy-flow"))
    );
    assert_eq!(
        snapshot.pointer("/10/WorkflowCodeArtifactUpdated/artifact/source"),
        Some(&serde_json::json!("workflow.workflow({ alias: 'toy' })"))
    );
    assert_eq!(
        snapshot.pointer("/11/WorkflowCodeArtifact/artifact/definition/workflow/alias"),
        Some(&serde_json::json!("toy"))
    );
    assert_eq!(
        snapshot.pointer("/12/WorkflowCodeArtifactsListed/artifacts/0/source_bytes"),
        Some(&serde_json::json!(37))
    );
    assert_eq!(
        snapshot.pointer("/13/WorkflowCodeArtifactDeleted/path"),
