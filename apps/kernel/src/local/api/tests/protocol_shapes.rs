...
fn local_daemon_protocol_waiting_room_activity_shape_is_versioned() {
    assert_eq!(LOCAL_DAEMON_PROTOCOL_VERSION, 188);
...
fn local_daemon_protocol_workflow_publication_shape_is_versioned() {
    ...
    assert_eq!(
        format!("{hash:x}"),
        "e3e679ef9c80abb1e4416a3c68e96ae6c912cab05e9a40d26525e644dd26df08"
    );
}
...
fn local_daemon_protocol_provider_run_usage_shape_is_versioned() {
    ...
    provider_run.set_resume_state(ProviderResumeState::from_pi_session_id("pi-session-1"));
    ...
    assert_eq!(
        snapshot.pointer("/ProviderRun/provider_run/resume_state/pi_session_id"),
        Some(&serde_json::json!("pi-session-1"))
    );
