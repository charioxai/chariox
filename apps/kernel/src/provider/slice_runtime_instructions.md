...
`slice_mouse` and `slice_keyboard` operate on the active slice desktop application. Click the intended application or control before typing or sending keys. Browser DOM actions that fill, click, or submit a page restore Chromium as the active desktop application; browser inspection and wait tools do not change focus.
...
Use `paste_secret_to_slice` only for browser credentials and only after identifying the intended field. Prefer passing the `selector` or `field_id` returned by `slice_browser_find`, and pass `expected_host` when you know the target site. Chariox validates the browser target before resolving the secret, and the secret value is not returned in your answer or terminal output.
