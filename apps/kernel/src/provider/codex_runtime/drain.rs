...
pub fn drain_codex_events(
    ...
    let completion_recovery_requested = codex_turn_should_backfill(
        run.endpoint_mode(),
...
pub(super) fn codex_authoritative_backfill_due(
    has_active_turn: bool,
    completion_recovery_requested: bool,
    last_backfill_at: Option<std::time::Instant>,
) -> bool {
    has_active_turn
        && match last_backfill_at {
            None => true,
            Some(last) => {
                completion_recovery_requested
                    && last.elapsed() >= CODEX_AUTHORITATIVE_BACKFILL_INTERVAL
            }
        }
}
