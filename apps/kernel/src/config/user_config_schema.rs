...
pub(super) fn entries() -> Vec<UserConfigSchemaEntry> {
    vec![
        ...
        entry("credential_vault.backend", "enum", &["os_keychain", "linux_keyutils", "process_memory"], true, false, "restart_required", "boot", "Vault storage backend for vault-backed credentials. process_memory is volatile and only intended for Arroba-managed slices."),
