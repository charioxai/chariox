...
impl CommandRouter {
    pub(crate) async fn persist_lease_worker_state(
        &self,
        callers: &crate::transport::relay_client::LeaseCallerAuthorization,
    ) -> Result<(), DaemonError> {
        self.app.lock().await.persist_lease_worker_state(callers)
    }
