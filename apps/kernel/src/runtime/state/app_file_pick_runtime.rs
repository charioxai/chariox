//! Protocol 354 file picks: each pending App file request is shown to its owner
//! as a trusted kernel prompt. The owner answers from a terminal with
//! `GrantAppFile` (the chosen files' bytes) or declines; App code, an App view
//! or an agent cannot answer.
use super::KernelRuntimeState;
use crate::durable_state::app_file_grants::{
    FileGrantCommand, FilePick, GrantedFile, MAX_FILES, MAX_FILE_BYTES,
};
use crate::local::{AppRequestErrorCode, GrantAppFileRequest, LocalDaemonResponse};
