...
#[tokio::test]
async fn running_metaagent_provider_without_active_turn_does_not_inject_orphaned_task() {
    let mut app = DaemonApp::bootstrap(crate::config::DaemonConfig::for_tests())
        .expect("daemon bootstrap should succeed");
    let (session, agent) = crate::app::KernelSessionService::new(&mut app)
        .create_session(crate::session::CreateSessionRequest::new(
            "workspace-meta-running-provider",
            "worktree-meta-running-provider",
        ))
        .expect("session should be created");
    let agent = app
        .agents_mut()
    ...
    app.sessions_mut()
        .start_or_update_metaagent_task(
            session.id(),
            agent.id(),
            "Keep watching while the provider run is still active.",
        )
        .expect("metaagent task should start");
    let run = app
        .launch_provider(
    ...
    app.sessions_mut()
        .set_active_provider_run(session.id(), Some(run.id().to_string()))
        .expect("provider run should be selected for the session");
    ...
    assert!(
        ...
        "test requires an active provider run"
    );
    assert!(
        !runtime.owned.active_turns.snapshot().contains_key(run.id()),
        "test exercises the provider-run fallback when no active turn is registered"
    );

    let completed = crate::session::PromptQueueItem::new(
        "completed-meta-prompt",
        "client-meta-running-provider",
        agent.id(),
        "completed meta turn",
        crate::session::PromptStatus::Completed,
    );
    runtime
        .inject_orphaned_metaagent_task_event_after_turn(
            session.id(),
            agent.id(),
        ...
        .expect("orphaned task injection check should succeed");
    ...
    assert!(
        runtime
            ...
            .list(agent.id(), Some("metaagent.task.orphaned"), None, 10)
            .is_empty(),
        "a running metaagent provider must suppress orphan recovery even when active-turn state is absent"
    );
}
