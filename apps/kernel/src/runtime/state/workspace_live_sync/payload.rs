//! JSON payload shaping for workspace live sync tool responses.
//!
...
pub(super) fn workspace_live_sync_warning_payload(
...
pub(super) fn workspace_live_sync_error_payload(error: crate::io::ArtifactEditError) -> serde_json::Value {
    match error {
        ...
        crate::io::ArtifactEditError::UnsupportedDomain { domain } => serde_json::json!({
            ...
            "domain": workspace_live_sync_domain_name(domain),
        }),
        ...
        crate::io::ArtifactEditError::ActiveReservationConflict {
            ...
            "path": path.to_string_lossy(),
            "active_owner": workspace_live_sync_reservation_owner_payload(active_owner),
            "requested_ranges": requested_ranges.into_iter().map(workspace_live_sync_range_payload).collect::<Vec<_>>(),
            "reserved_ranges": reserved_ranges.into_iter().map(workspace_live_sync_range_payload).collect::<Vec<_>>(),
            "message": message,
        }),
        crate::io::ArtifactEditError::Conflict {
            ...
            "requested_ranges": requested_ranges.into_iter().map(workspace_live_sync_range_payload).collect::<Vec<_>>(),
            "changed_ranges": changed_ranges.into_iter().map(workspace_live_sync_range_payload).collect::<Vec<_>>(),
            "message": message,
        }),
    }
}

pub(super) fn workspace_live_sync_reservation_owner_payload(
...
pub(super) fn workspace_live_sync_range_payload(range: crate::io::TextRange) -> serde_json::Value {
    serde_json::json!({
        "start": range.start,
        "end": range.end,
    })
...
pub(in crate::runtime::state) fn workspace_live_sync_daemon_error(
    error: crate::io::ArtifactEditError,
) -> DaemonError {
    DaemonError::LocalTransport {
        operation: "runtime_tool_workspace_live_sync",
        message: workspace_live_sync_error_payload(error).to_string(),
    }
}
