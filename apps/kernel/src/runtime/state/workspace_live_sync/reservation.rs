//! Workspace live sync reservation ownership and range policy.

use super::*;

pub(in crate::runtime::state) fn workspace_live_sync_reservation_ranges_for_operation(
    operation: &crate::io::AgentEditOperation,
    before: Option<&WorkspaceLiveSyncTextSnapshot>,
    ...
    coordinator
        ...
        .map_err(|reason| crate::transport::runtime_tools::RuntimeToolResult {
            ...
            payload: serde_json::json!({
                ...
                "next_action": "Another managed writer has reserved the same artifact area. Wait for that write to finish, reread the artifact with arroba.read_artifact, and retry through Arroba workspace live sync.",
            }),
        })
}

pub(in crate::runtime::state) fn workspace_live_sync_reservation_owner(
