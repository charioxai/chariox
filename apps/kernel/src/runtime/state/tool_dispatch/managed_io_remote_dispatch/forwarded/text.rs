...
pub(super) fn dispatch_forwarded_edit(
    ...
    let state =
        forwarded_artifact_state(artifact_states, &path, "forwarded_managed_io_edit_artifact")?;
...
pub(super) fn dispatch_forwarded_write(
    ...
    let result = coordinator.apply_edit(crate::io::ArtifactWriteRequest {
        ...
        intent: crate::io::AgentEditIntent {
            path: path.clone(),
            snapshot_id: managed_io_write_snapshot_id_from_arg(args.snapshot_id.clone(), &path),
...
fn forwarded_artifact_state<'a>(
    artifact_states: &'a [crate::transport::relay_peer::RemoteManagedIoArtifactState],
    path: &PathBuf,
    operation: &'static str,
) -> Result<&'a crate::transport::relay_peer::RemoteManagedIoArtifactState, DaemonError> {
