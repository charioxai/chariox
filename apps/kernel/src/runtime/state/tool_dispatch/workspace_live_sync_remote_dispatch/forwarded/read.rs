...
pub(super) fn dispatch_forwarded_read(
    ...
    let args = serde_json::from_value::<crate::transport::runtime_tools::WorkspaceLiveSyncReadArtifactArgs>(
        arguments,
    )
    ...
    let state = remote_workspace_live_sync_state_for_path(artifact_states, &PathBuf::from(&args.path))
