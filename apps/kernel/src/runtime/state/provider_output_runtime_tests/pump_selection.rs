...
#[tokio::test]
async fn provider_launch_finalization_failure_restores_parked_predecessor() {
    let mut app = DaemonApp::bootstrap(crate::config::DaemonConfig::for_tests())
        .expect("daemon bootstrap should succeed");
    let (session, agent) = crate::app::KernelSessionService::new(&mut app)
        .create_session(crate::session::CreateSessionRequest::new(
            "workspace-launch-finalization-rollback",
            "worktree-launch-finalization-rollback",
        ))
        .expect("session should be created");
    let predecessor = app
        .providers_mut()
        ...
        .expect("predecessor should launch");
    app.sessions
        .set_active_provider_run(session.id(), Some(predecessor.id().to_string()))
        .expect("predecessor should be active");
    app.update_provider_run_projection(predecessor.clone());
    ...
    runtime.inject_provider_launch_finalization_failure(started.run.id());
    ...
    assert_eq!(
        ...
        "the replacement must roll back after finalization fails"
    );
    assert_eq!(
        runtime
            .owned
        ...
        "the predecessor must remain resumable until finalization succeeds"
    );
    assert_eq!(
        runtime
            .owned
