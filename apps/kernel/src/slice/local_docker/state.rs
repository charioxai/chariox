...
fn save_local_docker_slice_state_inner(
    ...
    disk_admission::with_slice_snapshot_disk_admission(|admission| {
        with_local_docker_slice_snapshot_quiesced(
            record,
            options,
            quiesce,
            "slice.state.save",
...
fn with_local_docker_slice_snapshot_quiesced<T>(
    ...
    match quiesce {
        ...
        SliceSnapshotQuiesce::Desktop => {
            stop_local_docker_slice_desktop_for_snapshot(record, options)?
        }
    }
    let result = snapshot();
