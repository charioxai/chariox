...
import {
  buildPath1RemoteCellInvocation,
  parseArgs,
  runPath1RemoteCell,
} from "./live-remote-native-tui-drill.mjs"
import { PATH1_RUNNER_CONTEXT_SCHEMA } from "../../../scripts/path1-oss-runner-context-adapter.mjs"

const repoRoot = path.resolve(".")

function context(provider = "opencode") {
  const runId = "path1-remote-test-001"
  const sessionId = "home-session-remote-001"
  const roomId = "room-remote-001"
  return {
    schema: PATH1_RUNNER_CONTEXT_SCHEMA,
    runId,
    allocationId: "allocation-remote-001",
    machineId: "machine-remote-001",
    homeKernelId: "home-kernel-remote-001",
    homeRelayRealmId: "relay-realm-remote-001",
    repoRoot,
    ...
    journal: { ref: "journal-remote-001", sequence: 3 },
    runtimeBindings: {
...
function evidence(current) {
  return {
    ...
    capabilities: { remoteTui: { attached: true, sameRoom: true }, reconnect: { persistence: true } },
    receiptId: "receipt-remote-001",
  }
}

test("remote standalone defaults still request the existing all-provider scenarios", () => {
  const options = parseArgs([])
  assert.deepEqual(options.providers, ["opencode", "codex", "claude"])
  assert.equal(options.runnerContext, null)
})

test("remote Path 1 invocation carries the exact existing session and no relay credential", () => {
  const current = context()
  const invocation = buildPath1RemoteCellInvocation(current, current.provider)
...
test("injected remote clients bind both remote evidence and provider output to one Room", async () => {
  const current = context()
  const events = []
  const result = await runPath1RemoteCell({
    context: current,
    provider: current.provider,
  ...
  await assert.rejects(
    () => runPath1RemoteCell({
      context: { ...current, roomId: "stale-room-remote-001" },
      provider: current.provider,
      clientFactory: factory([]),
      processRunner: { async run() { throw new Error("stale remote result") } },
    }),
