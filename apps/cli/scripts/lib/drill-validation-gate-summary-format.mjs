...
export function formatDrillValidationGateSummary(report) {
  ...
  const requiredRuntimeSignalOwners = platform.requiredRuntimeSignalOwners ?? []
  const missingRuntimeSignalOwners = platform.missingRuntimeSignalOwners ?? []
  if (requiredRuntimeSignalOwners.length > 0) {
    lines.push(`platform_required_runtime_signal_owners=${requiredRuntimeSignalOwners.join(",")} missing=${missingRuntimeSignalOwners.join(",") || "none"}`)
  }
  ...
  const requiredArtifactGeneratedEvidenceRepos = artifacts.requiredArtifactGeneratedEvidenceRepos ?? []
  const missingArtifactGeneratedEvidenceRepos = artifacts.missingArtifactGeneratedEvidenceRepos ?? []
  if (requiredArtifactGeneratedEvidenceRepos.length > 0) {
    lines.push(`artifact_required_generated_evidence_repos=${requiredArtifactGeneratedEvidenceRepos.join(",")} missing=${missingArtifactGeneratedEvidenceRepos.join(",") || "none"}`)
  }
  ...
  if (artifacts.aggregate) {
    ...
    const requiredFailureClassifications = Object.entries(artifacts.aggregate.requiredFailureClassifications ?? {}).sort(([left], [right]) => left.localeCompare(right))
    if (requiredFailureClassifications.length > 0) {
      lines.push(`artifact_required_failure_classifications=${requiredFailureClassifications.map(([classification, count]) => `${classification}:${count}`).join(",")}`)
    }
    const missingFailureClassifications = Object.entries(artifacts.aggregate.missingFailureClassifications ?? {}).sort(([left], [right]) => left.localeCompare(right))
    if (missingFailureClassifications.length > 0) {
      lines.push(`artifact_missing_failure_classifications=${missingFailureClassifications.map(([classification, count]) => `${classification}:${count}`).join(",")}`)
    }
    const plannedOwners = Object.entries(artifacts.aggregate.plannedOwners ?? {}).sort(([left], [right]) => left.localeCompare(right))
    if (plannedOwners.length > 0) {
      lines.push(`artifact_planned_owners=${plannedOwners.map(([owner, count]) => `${owner}:${count}`).join(",")}`)
    }
    const plannedClassifications = Object.entries(artifacts.aggregate.plannedClassifications ?? {}).sort(([left], [right]) => left.localeCompare(right))
    if (plannedClassifications.length > 0) {
      lines.push(`artifact_planned_classifications=${plannedClassifications.map(([classification, count]) => `${classification}:${count}`).join(",")}`)
    }
    ...
    const generatedEvidenceRepos = Object.entries(artifacts.aggregate.generatedEvidenceRepos ?? {}).sort(([left], [right]) => left.localeCompare(right))
    if (generatedEvidenceRepos.length > 0) {
      lines.push(`artifact_generated_evidence_repos=${generatedEvidenceRepos.map(([repo, count]) => `${repo}:${count}`).join(",")}`)
    }
