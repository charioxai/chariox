...
test("fails and aggregates explicit failure manifest directories", async () => {
  const rootDir = await mkdtemp(path.join(os.tmpdir(), "chariox-validation-gate-failures-"))
  try {
    const runDir = path.join(rootDir, "failed-run")
    const manifestPath = path.join(runDir, "chariox-drill-failure.json")
...
test("discovers failure manifests below configured roots", async () => {
  const rootDir = await mkdtemp(path.join(os.tmpdir(), "chariox-validation-gate-failures-"))
  try {
    const first = path.join(rootDir, "target", "run-one", "chariox-drill-failure.json")
    const second = path.join(rootDir, ".artifacts", "run-two", "chariox-drill-failure.json")
