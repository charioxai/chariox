  ...
  return {
    ...
    nodePrompt() {
      return [
        ...
        `After the MCP tool call succeeds, use Chariox workspace live sync to create \`outputs/${markerFile}\` with exactly \`ECHO:M7_WORKFLOW_ECHO_OK\`.`,
        'Then call the Chariox runtime MCP tool `validate_and_submit_workflow_run_output` with workflow_output_json exactly `{"echo":"ECHO:M7_WORKFLOW_ECHO_OK"}`.',
