...
const KNOWN_RED_CASES = new Map([
  ["browser.authentication", "OAuth popup and reauthentication require the later multi-target browser path"],
  ...
  ["fault.disk-pressure", "disk-pressure admission and save recovery belong to the later persistence fault lane"],
  ["fault.resource-exhaustion", "process and file-descriptor admission belong to the later resource fault lane"],
])
...
export function buildRedCapCommandPlan({
  ...
  const env = {
    ...
    CHARIOX_SLICE_VIEWER_BACKEND: "novnc",
...
export function buildFaultCommand(plan, faultId, { staleUrl } = {}) {
  ...
  const processPattern = {
    ...
    "streamer-crash": `websockify.*${plan.ports.novnc}`,
    "browser-crash": "chromium.*--user-data-dir=",
  }[faultId]
  if (processPattern) {
    return {
      ...fault,
...
export function validateRedCapEvidenceManifest(manifest, { repoRoot } = {}) {
  ...
  for (const caseId of manifest.caseIds) {
    if (!CASE_DEFINITIONS.has(caseId)) throw new Error(`manifest.caseIds contains unknown case ${caseId}`)
  }
  ...
  const expectedAssertionKeys = manifest.caseIds.flatMap((caseId) => CASE_DEFINITIONS.get(caseId).assertions.map((id) => `${caseId}:${id}`))
  const actualAssertionKeys = manifest.assertions.map((assertion) => `${assertion.caseId}:${assertion.id}`)
  if (JSON.stringify(actualAssertionKeys) !== JSON.stringify(expectedAssertionKeys)) {
    throw new Error("manifest.assertions must cover every declared case exactly once in catalog order")
  }
...
export function redactRedCapCommandArgs(args = []) {
  const sanitized = [...args]
  for (let index = 0; index < sanitized.length - 2; index += 1) {
    if (sanitized[index] !== "browser-fill") continue
    if (!/password|secret/i.test(String(sanitized[index + 1]))) continue
    sanitized[index + 2] = "<redacted>"
  }
  return sanitized
}
