...
test("red-cap profiles and command construction are bounded and use the pre-cutover stack", () => {
  ...
  assert.equal(plan.provision.env.CHARIOX_SLICE_VIEWER_BACKEND, "novnc")
...
test("fault controls have deterministic order, exact targets, and bounded commands", () => {
  ...
  assert.match(buildFaultCommand(plan, "streamer-crash").args.join(" "), new RegExp(String(plan.ports.novnc)))
  assert.match(buildFaultCommand(plan, "browser-crash").args.join(" "), /--user-data-dir=/)
...
test("evidence validation requires exact case coverage, monotonic time, commands, resources, and cleanup", async () => {
  ...
  try {
    ...
    assert.throws(() => validateRedCapEvidenceManifest(incomplete, { repoRoot: REPO_ROOT }), /cover every declared case/)
  } finally {
...
test("command evidence redacts password and secret browser-fill values", () => {
  assert.deepEqual(
    redactRedCapCommandArgs([
      "docker",
      "exec",
