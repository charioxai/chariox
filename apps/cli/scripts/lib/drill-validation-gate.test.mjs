...
test("passes with valid platform bundle and complete matrix reports", async () => {
  ...
  try {
    ...
    assert.deepEqual(report.checks.platformBundle.validationSuite, {
      testCount: 35,
      coverageAreas: [
        { id: "artifact-contracts", testCount: 10 },
        { id: "failure-diagnostics", testCount: 3 },
        { id: "matrix-validation", testCount: 13 },
    ...
    assert.match(formatDrillValidationGateSummary(report), /platform_validation_suite_tests=35 coverage=artifact-contracts:10/)
  } finally {
