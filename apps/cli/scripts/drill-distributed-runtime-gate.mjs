...
async function main() {
  ...
  try {
    ...
    const report = await runDrillValidationGate({
      ...
      requiredArtifactRuntimeAuthorityInvariants: options.requiredArtifactRuntimeAuthorityInvariants,
...
function parseArgs(argv) {
  ...
  if (options.requiredGeneratedValidationSuiteFailureRoots.length > 0) {
    throw new Error("--require-generated-validation-suite-failure-root is supported by drill-validation-gate-summary.mjs after a distributed gate report is written")
  }
  return options
}
...
function generatedEvidenceMetadataFor(generatedEvidence) {
  ...
  const matrixArtifactIndexes = (generatedEvidence.matrixReports.artifactIndexes ?? [])
    ...
    .sort()
  const validationSuiteArtifactIndexes = (generatedEvidence.validationSuites.artifactIndexes ?? [])
    .filter((artifactIndex) => typeof artifactIndex === "string" && artifactIndex.length > 0)
    .sort()
