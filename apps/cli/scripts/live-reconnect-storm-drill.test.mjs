...
test("reconnect storm drill requires isolated slow-lane closure", async () => {
  ...
  assert.match(source, /providerTurnAlreadySettled = true/)
  assert.match(source, /message\.includes\("has no active prompt"\)/)
  ...
  assert.match(source, /assertOnlySlowSubscriptionClosed\(relayHealth, clientCount, seen.slice\(1\), postCloseMarker\)/)
  assert.match(source, /every healthy subscriber to receive output after slow isolation/)
  assert.match(source, /assert.deepEqual\(resumeCounts.slice\(1\), healthyResumeBaseline/)
  ...
  assert.match(source, /const \[, healthyResults\] = await Promise\.all\(\[/)
  assert.doesNotMatch(source, /await withDeadline\(slowFloodProgress[\s\S]*const healthyResults = await healthyWork/)
