...
const DEFAULT_PROVIDER = process.env.CHARIOX_METAAGENT_WORKFLOW_WEBAPP_PROVIDER ?? 'codex'
const DEFAULT_MODEL = process.env.CHARIOX_METAAGENT_WORKFLOW_WEBAPP_MODEL ?? 'gpt-5.5'
const DEFAULT_EFFORT = process.env.CHARIOX_METAAGENT_WORKFLOW_WEBAPP_EFFORT ?? 'medium'
  ...
  while (Date.now() < deadline) {
    ...
    if (providerRunId) {
      ...
      if (run?.agent_instance_id === agentId || run?.agent_id === agentId) {
        const active = await waitForProviderRun(client, requests, providerRunId, timeoutMs, pollMs)
...
async function main() {
  ...
  const env = {
    ...
    CHARIOX_DAEMON_ID: `${drillSlug}-${process.pid}-${Date.now()}`,
  ...
  try {
    ...
    const metaPrompt = `/meta ${taskPrompt}`
