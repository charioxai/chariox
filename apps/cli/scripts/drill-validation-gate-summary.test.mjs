...
test("drill validation gate summary help lists artifact coverage requirements", async () => {
  ...
  assert.match(stdout, /--require-artifact-generated-matrix-name NAME/)
  assert.match(stdout, /--require-artifact-generated-matrix-repo REPO/)
  assert.match(stdout, /--require-artifact-generated-validation-suite-artifact-index PATH/)
  assert.match(stdout, /--require-artifact-generated-validation-suite-failure-root ROOT/)
...
test("drill validation gate summary consumes artifact metadata inputs", async () => {
  ...
  try {
    ...
    assert.deepEqual(stdoutAggregate.artifactCoverageInputs[0].artifactCoverage.requiredArtifactValidationPresets, ["distributed-runtime"])
    assert.deepEqual(stdoutAggregate.artifactCoverageInputs[0].artifactCoverage.missingArtifactValidationPresets, [])
