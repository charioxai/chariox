...
try {
  ...
  if (plan || paths?.tempRoot) {
    ...
    commands.push(...cleanup.commands)
  }
  if (runError) {
...
async function runLiveDrill() {
  ...
  createdContainer = (await commandRunner("docker", ["inspect", plan.containerName], { timeoutMs: 10_000, label: "ownership-container-inspect" })).code === 0
  createdVolume = (await commandRunner("docker", ["volume", "inspect", plan.volumeName], { timeoutMs: 10_000, label: "ownership-volume-inspect" })).code === 0
  fixture = await startBrowserComputerFixture({
    host: "0.0.0.0",
    port: 0,
    password: `m0-in-process-${paths.runId}`,
  })
  ...
  await runBrowserCases(plan, fixtureUrl, capturedStack)
...
async function runBrowserCases(plan, fixtureUrl) {
