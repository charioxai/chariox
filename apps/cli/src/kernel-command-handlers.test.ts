...
test("kernel health formatter renders provider-run invariants", () => {
  ...
  assert.match(rendered, /remote runtime invariants: worker_runs=ok; manifests=settled; live_sync_scope=selected-workspace-only/)
...
test("kernel health formatter reports provider-run identity issues", () => {
  ...
  assert.match(rendered, /next: run \/agent inspect agent-1; run \/provider processes; capture a debug bundle, then stop duplicate provider runs/)
...
test("kernel health formatter reports remote execution issues", () => {
  ...
  assert.match(rendered, /remote runtime invariants: worker_runs=attention missing_worker_runs=1 malformed=1; manifests=settled; live_sync_scope=selected-workspace-only/)
...
test("kernel health command reports duplicate provider-run bindings", async () => {
  ...
  assert.match(notices.at(-1) ?? "", /next: run \/agent inspect agent-1; run \/provider processes; capture a debug bundle, then stop duplicate provider runs/)
...
test("kernel remote-runtime command opens health projection with remote footer", async () => {
  ...
  assert.match(notices.at(-1) ?? "", /remote runtime invariants: worker_runs=ok; manifests=settled; live_sync_scope=selected-workspace-only/)
...
test("kernel remote-runtime formatter reports settling manifests as degraded attention", async () => {
  ...
  assert.match(rendered, /remote runtime invariants: worker_runs=ok; manifests=attention syncing=1 pending=1 failed=0 stale=0 missing=0 pending_revoke=0; live_sync_scope=selected-workspace-only/)
