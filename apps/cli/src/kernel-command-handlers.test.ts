...
test("kernel health formatter reports remote execution issues", () => {
  ...
  assert.match(rendered, /next: run \/agent inspect agent-remote; run \/machine kernels worker-machine; reconnect or relaunch/)
})
...
test("kernel health formatter reports remote extension sync issues", () => {
  ...
  assert.match(rendered, /next: run \/extension sync-status agent-failed; use \/extension sync-retry agent-failed after worker connectivity is healthy/)
})
