...
test("waiting room session rows render active sessions with stable columns", () => {
  ...
  assert.equal(rows[1]?.columns?.[3]?.trim(), "run /kernel remote-runtime; run /agent inspect <agent>; reconnect or relaunch the remote/slice worker before sending prompts to that remote/slice agent")
