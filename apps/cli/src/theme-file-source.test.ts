...
test("themeDirectories uses XDG config and workspace theme locations", () => {
  ...
  try {
    process.env.XDG_CONFIG_HOME = "/tmp/chariox-config"
    assert.deepEqual(themeDirectories("/workspace/project"), [
      "/tmp/chariox-config/chariox/themes",
      "/workspace/project/.chariox/themes",
    ])
  } finally {
...
test("themeFiles returns sorted json files only", async () => {
  const root = path.join(os.tmpdir(), `chariox-theme-source-${Date.now()}`)
