...
test("native TUI runtime banner shows ownership, placement, worktree, and live sync", () => {
  ...
  assert.match(banner, /home ext sync: pending/)
  assert.match(banner, /home ext next: home keeps stale home-proxy calls blocked until the worker manifest settles; run \/extension sync-status A1; run \/machine kernels hetzner if it does not settle; use \/extension sync-retry A1 after worker connectivity is healthy/)
...
test("remote native TUI banner does not invent manifest sync for worker-local MCPs and skills", () => {
  ...
  assert.doesNotMatch(banner, /(?:home|worker) ext sync:/)
})
...
test("native TUI runtime banner renders local defaults without inventing state", () => {
  ...
  assert.match(banner, /placement:      home-local/)
  assert.match(banner, /live sync:      config default/)
  assert.match(banner, /extensions:     none/)
  assert.match(banner, /ext runtime:    home-local: definition, credentials, and execution stay on the home kernel/)
  assert.match(banner, /ext boundary:   home resolves, validates, and executes locally; credentials stay on home/)
  assert.doesNotMatch(banner, /(?:home|worker) ext sync:/)
...
test("native TUI runtime banner surfaces blocked remote extension sync", () => {
  ...
  assert.match(banner, /ext boundary:   home validates every call; credentials never leave home/)
  assert.match(banner, /home ext sync: failed, pending revoke, hash=abcdef123456, error=worker offline/)
  assert.match(banner, /home ext next: keep the home revoke in place; run \/extension sync-status A1; run \/machine kernels hetzner if the revoke stays pending; use \/extension sync-retry A1 after the worker reconnects/)
})

test("native TUI runtime banner keeps Worker-only sync and recovery separate from Home", () => {
  const banner = formatNativeTuiRuntimeBanner({
    surface: "codex native-tui",
    session: session({ workspace_live_sync_mode: "managed" }),
    agent: agent({
      ...
      extension_grants: [{ source: "worker", kind: "mcp", name: "filesystem" }],
  ...
  assert.match(banner, /extensions:     mcp=filesystem \(worker-local\)/)
  assert.match(banner, /worker ext sync: failed, pending revoke, hash=worker-hash, error=worker offline/)
  assert.match(banner, /worker ext next: keep the Worker revoke in place;/)
  assert.doesNotMatch(banner, /home ext sync:/)
  assert.doesNotMatch(banner, /home ext next:/)
})

test("native TUI runtime banner renders both sync lanes for mixed-source grants", () => {
  const banner = formatNativeTuiRuntimeBanner({
    surface: "codex native-tui",
    session: session({ workspace_live_sync_mode: "managed" }),
    agent: agent({
      ...
      extension_grants: [
        { source: "home", kind: "connector", name: "status-api" },
        { source: "worker", kind: "mcp", name: "filesystem" },
      ],
  ...
  assert.match(banner, /home ext sync: synced/)
  assert.match(banner, /worker ext sync: pending/)
  assert.match(banner, /worker ext next: Worker-local grants are not settled on the worker;/)
})
...
test("native TUI runtime banner keeps final revokes visible after grants are gone", () => {
  ...
  assert.match(banner, /home ext sync: failed, pending revoke, hash=empty-hash, error=worker offline/)
  assert.match(banner, /home ext next: keep the home revoke in place; run \/extension sync-status A1; run \/machine kernels hetzner if the revoke stays pending; use \/extension sync-retry A1 after the worker reconnects/)
})
