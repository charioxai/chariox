...
test("parseArgs help lists remote runtime once next to kernel health", () => {
  ...
  assert.match(output, /\/extension catalog\s+list home and worker extension catalogs for an agent/)
  assert.match(output, /\/extension grant\s+grant an extension from home or worker to an agent/)
  assert.match(output, /\/extension revoke\s+revoke a home or worker extension from an agent/)
  assert.match(output, /\/extension grants\s+show source, runtime placement, and kernel for grants/)
  assert.match(output, /\/extension sync-status\s+show home-proxy and worker-local sync and recovery for an agent/)
