...
import type { SessionHistoryPageEntry } from "./kernel-types.js"
...
test("extractPromptHistoryEntries preserves attachment metadata while merging prompt fragments", () => {
  const entries: SessionHistoryPageEntry[] = [
    {
      ...
      entry: {
        ...
        attachments: [{
          ...
          mime: "image/png",
          filename: "a.png",
          preview_url: "file:///tmp/a-thumb.png",
        }],
      },
    },
    {
  ...
  assert.deepEqual(extractPromptHistoryEntries(entries), ["look here"])
  assert.equal(entries[0]?.entry.attachments?.[0]?.preview_url, "file:///tmp/a-thumb.png")
})
