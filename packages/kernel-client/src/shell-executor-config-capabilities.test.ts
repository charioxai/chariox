...
test("executeShellCommand shows remote extension sync diagnostics", async () => {
  ...
  assert.match(grantsResult.message ?? "", /next: keep the home revoke in place; run \/machine kernels machine-1; run \/extension sync-retry agent-1 after the worker reconnects/)
  ...
  assert.match(statusResult.message ?? "", /next: keep the home revoke in place; run \/machine kernels machine-1; run \/extension sync-retry agent-1 after the worker reconnects/)
  assert.equal(retryResult.ok, true)
  assert.match(retryResult.message ?? "", /manifest hash: hash-1/)
  assert.match(retryResult.message ?? "", /next: keep the home revoke in place; run \/machine kernels machine-1; run \/extension sync-retry agent-1 after the worker reconnects/)
