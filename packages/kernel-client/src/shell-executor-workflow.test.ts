...
test("executeShellCommand requires confirmation for workflow Home proxy grants", async () => {
  const workflow = makeWorkflow({ nodes: [{ id: "node-1", agent_id: "agent-1" }] })
  ...
  const blocked = await executeShellCommand(
    parseShellCommand("workflow node extension grant node-1 script lookup --environment py"),
    context,
    { client: fake.client },
  )
  const confirmed = await executeShellCommand(
    parseShellCommand("workflow node extension grant node-1 script lookup --environment py --confirm-home-proxy"),
    context,
    { client: fake.client },
  )

  assert.equal(blocked.ok, false)
  assert.match(blocked.message ?? "", /Confirm exposing script lookup to remote agent agent-1/)
  assert.match(blocked.message ?? "", /rerun: workflow node extension grant node-1 script lookup --environment py --confirm-home-proxy/)
