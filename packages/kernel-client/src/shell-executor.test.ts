...
test("executeShellCommand renders shell-local context and pwd", async () => {
  ...
  assert.match(contextResult.message ?? "", /remote extension next: run \/extension sync-status agent-1; run \/machine kernels slice-machine; use \/extension sync-retry agent-1/)
...
test("executeShellCommand manages workspace links", async () => {
  ...
  assert.match(attachResult.message ?? "", /workspace sync managed.*workspace sync tracked.*managed write fencing/)
  ...
  assert.match(syncLinkResult.message ?? "", /workspace sync managed.*workspace sync tracked.*managed write fencing/)
